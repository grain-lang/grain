...
let is_ref : Types.value_description -> bool = function
  | { Types.val_kind =
        Types.TValPrim { Primitive.prim_name = "%makemutable";
                          prim_arity = 1 } } ->
        true
  | _ -> false
  ...
  *)
  let rec classify_expression env e = match e.exp_desc with
    ...
    | TExpBlock elts ->
        ...
        match elts with
        | [] -> Static
        | elts -> classify_expression env (get_last elts)
        end

    | TExpConstruct (_, {cstr_tag = CstrUnboxed}, [e]) ->
        classify_expression env e
    | TExpConstruct _ ->
        Static

    | TExpApp ({exp_desc = TExpIdent (_, _, vd)}, _)
      when is_ref vd ->
        Static
    | TExpApp _ ->
        Dynamic
...
let remove_pat pat env =
  Env.remove_list (pattern_bound_idents pat) env
...
let rec expression : Typedtree.expression -> term_judg =
  fun exp -> match exp.exp_desc with
    | TExpIdent (pth, _, _) ->
      path pth
    | TExpLet (rec_flag, bindings, body) ->
      (*
         G  |- <bindings> : m -| G'
         G' |- body : m
         -------------------------------
    ...
    | TExpApp ({exp_desc = TExpIdent (_, _, vd)}, args)
      when is_ref vd ->
      (*
        G |- e: m[Guard]
        ------------------
    ...
    | TExpApp (e, args)  ->
        ...
        in
        join [expression e; list expression args] << app_mode
    | TExpTuple exprs ->
    ...
    | TExpConstruct (_, desc, exprs) ->
      let access_constructor = empty
      ...
      let m' = match desc.cstr_tag with
        | CstrUnboxed ->
          Return
        | CstrConstant _ | CstrBlock _ (* | Cstr_extension _ *) ->
          Guard
      in
      join [
        access_constructor;
    ...
    | TExpBlock exprs ->
      ...
      *)
      join (List.fold_right (fun cur acc ->
        match acc with
        | [] -> [expression cur]
        | _ -> (expression cur << Guard)::acc
      ) exprs [])
    | TExpWhile (cond, body) ->
      (*
...
(* and structure_item : Typedtree.toplevel_stmt -> bind_judg =
  fun s m env -> match s.ttop_desc with
    | Tstr_eval (e, _) ->
      (*
        Ge |- e: m[Guard]
        G |- items: m -| G'
...
(* and open_declaration : Typedtree.import_declaration -> bind_judg =
...
*)
and case : Typedtree.match_branch -> mode -> Env.t * mode =
  fun { Typedtree.mb_pat; (* c_guard; *) mb_body } ->
    (*
    ...
    *)
    ...
    (fun m ->
       let env = judg m in
       (remove_pat mb_pat env), Mode.compose m (pattern mb_pat env))
...
and is_destructuring_pattern : Typedtree.pattern -> bool =
  fun pat -> match pat.pat_desc with
    | TPatAny -> false
    | TPatVar (_, _) -> false
    | TPatAlias (pat, _, _) -> is_destructuring_pattern pat
    | TPatConstant _ -> true
    | TPatTuple _ -> true
    | TPatConstruct (_, _, _) -> true
    (* | TPatVariant _ -> true *)
