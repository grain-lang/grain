...
For an overview of the theory of regular expressions in general, readers are referred
...
- `[:graph:]` - Matches all ASCII characters which use ink when printed (Unsupported until [grain-lang/grain#661](https://github.com/grain-lang/grain/issues/661) is resolved)
- `[:print:]` - Matches space, tab, and all ASCII ink users (Unsupported until [grain-lang/grain#661](https://github.com/grain-lang/grain/issues/661) is resolved)
...
until [grain-lang/grain#661](https://github.com/grain-lang/grain/issues/661) is resolved,
matches using `\p` and `\P` are disabled,
