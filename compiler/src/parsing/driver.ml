...
type error = AmbiguousParse of Parsetree.parsed_program list | NoValidParse

let report_error ppf = function
  | AmbiguousParse parses ->
      if
        !Grain_utils.Config.verbose
        || !Grain_utils.Config.parser_debug_level > 0
      then
        Format.fprintf ppf
          "The Grain compiler had trouble parsing your program. Here were the \
           potential parses:@\n\
           @[<v>%a@]"
          (fun ppf ->
            List.iter (fun x ->
  ...
  | NoValidParse ->
      Format.fprintf ppf
        "The Grain compiler was unable to parse your program. If you see this \
         message, please file an issue at https://github.com/grain-lang/grain"
...
let parse ?name lexbuf : Parsetree.parsed_program =
  ...
  let loc_start =
    Option.map_default
      (fun n -> { lexbuf.lex_start_p with pos_fname = n })
      lexbuf.lex_start_p name
  in
