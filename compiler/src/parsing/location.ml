...
let sexp_of_position (p : position) =
  Sexplib.Sexp.List
    [
      ...
      Sexplib.Sexp.List
        [ Sexplib.Sexp.Atom "file"; Sexplib.Conv.sexp_of_string p.pos_fname ];
      Sexplib.Sexp.List
        [ Sexplib.Sexp.Atom "line"; Sexplib.Conv.sexp_of_int p.pos_lnum ];
      Sexplib.Sexp.List
        [ Sexplib.Sexp.Atom "col"; Sexplib.Conv.sexp_of_int p.pos_cnum ];
      Sexplib.Sexp.List
        [ Sexplib.Sexp.Atom "bol"; Sexplib.Conv.sexp_of_int p.pos_bol ];
    ]
...
let position_of_yojson (yj : Yojson.Safe.t) : (position, string) result =
  ...
  | `Assoc contents -> (
      ...
      if not (List.for_all (Hashtbl.mem map) [ "file"; "line"; "col"; "bol" ])
      then
        Result.Error
        ...
        let file, line, col, bol =
          match
            List.map (Hashtbl.find map) [ "file"; "line"; "col"; "bol" ]
          with
          | [ a; b; c; d ] -> (a, b, c, d)
...
let sexp_of_t loc =
  if loc = dummy_loc then Sexplib.Sexp.Atom "dummy_loc" else sexp_of_t loc
...
let setup_colors () =
  Misc.Color.setup
  @@ Some
       ( if !Grain_utils.Config.color_enabled then Misc.Color.Auto
       else Misc.Color.Never )
...
let print_loc ppf loc =
  ...
  if startchar >= 0 then
    if line = endline then
      ...
      fprintf ppf "%s%i%sline %i%s%i" msg_char startchar msg_to endline msg_char
        endchar;
  fprintf ppf "@}"
...
let print_compact ppf loc =
  ...
  if startchar >= 0 then
    if line = endline then fprintf ppf ",%i--%i" startchar endchar
    else fprintf ppf ",%i--%i,%i" startchar endline endchar

let print_error ppf loc = fprintf ppf "%a%t:" print loc print_error_prefix
...
let pp_ksprintf ?before k fmt =
  ...
  (match before with None -> () | Some f -> f ppf);
  kfprintf
    (fun _ ->
      pp_print_flush ppf ();
...
let errorf ?(loc = dummy_loc) ?(sub = []) ?(if_highlight = "") fmt =
  pp_ksprintf ~before:print_phantom_error_prefix
...
let report_error ppf err = print_updating_num_loc_lines ppf !error_reporter err
...
let raise_errorf ?(loc = dummy_loc) ?(sub = []) ?(if_highlight = "") =
  pp_ksprintf ~before:print_phantom_error_prefix (fun msg ->
