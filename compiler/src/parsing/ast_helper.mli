...
module Pat : sig
  ...
  val record :
    ?loc:loc -> ((id * pattern) option * Asttypes.closed_flag) list -> pattern
...
module Exp : sig
  ...
  val let_ :
    ?loc:loc -> rec_flag -> value_binding list -> expression -> expression
...
module Top : sig
  ...
  val let_ :
    ?loc:loc -> export_flag -> rec_flag -> value_binding list -> toplevel_stmt
