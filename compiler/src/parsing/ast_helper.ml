...
let with_default_loc l = with_default_loc_src (fun () -> l)

let ident_empty = { txt = Identifier.IdentName "[]"; loc = !default_loc_src () }

let ident_cons =
  { txt = Identifier.IdentName "[...]"; loc = !default_loc_src () }
...
module Typ = struct
  let mk ?loc d =
    let loc = match loc with None -> !default_loc_src () | Some l -> l in
  ...
  let force_poly t =
    match t.ptyp_desc with PTyPoly _ -> t | _ -> poly ~loc:t.ptyp_loc [] t
end
...
module Pat = struct
  ...
  let record ?loc a =
    let patterns, closed =
      List.fold_right
        (fun (pat_opt, closed) (pats, closed_acc) ->
          ( Option.map_default (fun pat -> pat :: pats) pats pat_opt,
            if closed_acc = Asttypes.Open then Asttypes.Open else closed ))
        a ([], Asttypes.Closed)
    in
  ...
  let mk ?loc exports =
    ...
    List.map
      (fun (name, alias) ->
        ...
        if Str.string_match r name.txt 0 then ExportData desc
