...
let prepare_error =
  ...
  | MultipleModuleName loc -> errorf ~loc "Multiple modules in identifier"
...
type well_formedness_checker = { errs : wferr list ref; iterator : iterator }
...
let malformed_strings errs super =
  let iter_expr self ({ pexp_desc = desc; pexp_loc = loc } as e) =
    ...
    | PExpConstant (PConstString s) -> (
        ...
        with BatUTF8.Malformed_code -> errs := MalformedString loc :: !errs )
    | _ -> () );
    super.expr self e
  in
...
let modules_have_correct_case errs super =
  ...
  let iter_mods self imports =
    List.iter
      (fun { pimp_mod_alias = alias } ->
        match alias with
        | Some { loc = name_loc; txt = IdentName name } ->
            check_uppercase name_loc name
        | Some _ (* IdentExternal handled by another WF rule *) | None -> ())
      imports;
    super.import self imports
  in
    ...
    | Identifier.IdentExternal _ ->
        errs :=
          ModuleImportNameShouldNotBeExternal
            (Identifier.string_of_ident id, loc)
          :: !errs
  in
  let iter_mods self imports =
    List.iter
...
let no_empty_record_patterns errs super =
  ...
  in
  let iterator =
    { super with toplevel = iter_toplevel_binds; expr = iter_binds }
  in
  { errs; iterator }
...
let compose_well_formedness { errs; iterator } cur = cur errs iterator
...
let well_formedness_checker () =
  List.fold_left compose_well_formedness
