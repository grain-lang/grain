...
type t = IdentName of string | IdentExternal of t * string [@@deriving sexp]
...
let rec compare i1 i2 =
  ...
  | IdentExternal (mod1, n1), IdentExternal (mod2, n2) ->
      ...
      if n_comp <> 0 then n_comp else compare mod1 mod2
  | IdentName _, IdentExternal _ | IdentExternal _, IdentName _ ->
      String.compare (string_of_ident i1) (string_of_ident i2)

let last = function IdentName s -> s | IdentExternal (_, s) -> s
...
let unflatten = function
  | [] -> None
  | hd :: tl ->
      Some (List.fold_left (fun p s -> IdentExternal (p, s)) (IdentName hd) tl)

let parse s =
  match unflatten (split_at_dots s 0) with None -> IdentName "" | Some v -> v
