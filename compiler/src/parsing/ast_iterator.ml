...
module Ex = struct
  let iter sub exports =
    List.iter
      (fun export ->
        match export with
        | ExportData { pex_loc = loc } | ExportValue { pex_loc = loc } ->
            sub.location sub loc)
      exports
  ...
  let iter_export_all sub excepts =
    List.iter
      (fun except ->
        match except with
        | ExportExceptData name | ExportExceptValue name -> iter_loc sub name)
      excepts
end

module ExD = struct
  let iter_export_data_all sub excepts = List.iter (iter_loc sub) excepts
end
