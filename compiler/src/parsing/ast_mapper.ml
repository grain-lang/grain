...
module E = struct
  let map sub { pexp_desc = desc; pexp_loc = loc } =
    ...
    | PExpArrayGet (a, i) -> array_get ~loc (sub.expr sub a) (sub.expr sub i)
    | PExpArraySet (a, i, arg) ->
        array_set ~loc (sub.expr sub a) (sub.expr sub i) (sub.expr sub arg)
    | PExpRecord es ->
        record ~loc
          (List.map
             (fun (name, expr) -> (map_loc sub name, sub.expr sub expr))
             es)
    | PExpRecordGet (e, f) -> record_get ~loc (sub.expr sub e) (map_loc sub f)
    | PExpLet (r, vbs, e) ->
        let_ ~loc r (List.map (sub.value_binding sub) vbs) (sub.expr sub e)
    | PExpMatch (e, mbs) ->
        match_ ~loc (sub.expr sub e) (List.map (sub.match_branch sub) mbs)
...
module P = struct
  let map sub { ppat_desc = desc; ppat_loc = loc } =
    ...
    | PPatRecord (fs, c) ->
        record ~loc
          (List.map
             (fun (id, pat) -> (Some (map_loc sub id, sub.pat sub pat), c))
             fs)
  ...
  let map_one sub
    ...
    let ival =
      ...
      | PImportValues values ->
          PImportValues
            (List.map
               (fun (name, alias) -> (map_loc sub name, map_opt sub alias))
               values)
