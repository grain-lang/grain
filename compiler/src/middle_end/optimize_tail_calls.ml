...
type analysis += TailCallOptimized of bool
...
let get_rewrite_rule id =
  try Ident_tbl.find rewrite_rules id with Not_found -> id
...
module TailCallsArg : Anf_mapper.MapArgument = struct
  ...
  let create_lambda_runner lambda
      { continue_id; next_id; first_f_id; arg_ref_ids } =
    let assign_id id value =
      wrap_comp lambda @@ CAssign (wrap_id lambda id, wrap_id lambda value)
    in
    let assign_imm id value =
      wrap_comp lambda @@ CAssign (wrap_id lambda id, wrap_imm lambda value)
    in
    ...
    let sequence comp_exprs anf_expr =
      List.fold_right
        (fun comp seq -> wrap_anf_with_comp lambda @@ AESeq (comp, seq))
        comp_exprs anf_expr
    in
    let to_anf value = wrap_anf_with_comp lambda @@ AEComp value in

    let lambda_args =
      match lambda.comp_desc with
      | CLambda (args, _) -> args
    ...
    let arg_ref_assignments =
      List.map2
        (fun arg_ref arg -> assign_id arg_ref arg)
        arg_ref_ids lambda_args
    in
           ...
           [
             ...
             @@ CWhile
                  ( unbox continue_id |> to_anf,
                    ...
                    @@ bind next_result_id
                         ( CApp (wrap_id lambda unboxed_next_id, [])
                         |> wrap_comp lambda )
    ...
    | AELet (global, Recursive, binds, body)
      when has_tail_recursive_binding binds ->
        ...
        let new_binds =
          List.flatten
          @@ List.map
               (fun ((id : Ident.t), ({ comp_desc } as bind)) ->
                 ...
                 | CLambda (args, body) when comp_is_tail_recursive bind ->
                     ...
                     let arg_refs =
                       List.map
                         (fun id -> (id, wrap_comp default_ref))
                         arg_ref_ids
                     in
                     ...
                     let iterative_lam_id =
                       Ident.create (id.name ^ "#tc_iter")
                     in
                     ...
                     let iterative_lam =
                       ...
                       @@ CLambda
                            ( [],
                              wrap_anf
                              @@ AELet
                                   (Nonglobal, Nonrecursive, arg_derefs, body)
                            )
                     in
  ...
  let enter_comp_expression ({ comp_desc = desc } as c) =
    ...
    | CApp ({ imm_desc = ImmId f }, args)
      when has_transform_rule f && comp_is_tail_call c ->
        ...
        let replaced_tail_call =
          List.fold_right2
            (fun new_f_arg arg seq ->
              wrap_anf_with_comp c
              @@ AESeq (wrap_comp c @@ CAssign (wrap_id c new_f_arg, arg), seq))
            new_f_args args set_break_and_next
        in
        {
          c with
          comp_desc =
    ...
    | CLambda (args, _) ->
        List.iter (fun arg -> pop_rewrite_rule arg) args;
        c
    | _ -> c
