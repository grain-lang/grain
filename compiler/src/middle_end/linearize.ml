...
let lookup_symbol mod_ mod_decl name original_name =
  ...
  | None ->
      ...
      ( match mod_decl.md_filepath with
      | Some filepath ->
          value_imports :=
...
let extract_bindings pat cexpr =
  List.map
    (fun (name, e) -> BLet (name, e))
    (MatchCompiler.extract_bindings pat cexpr)

let transl_const (c : Types.constant) :
    (imm_expression, string * comp_expression) either =
  match c with
...
type item_get = RecordPatGet of int | TuplePatGet of int

let rec transl_imm
    ({ exp_desc; exp_loc = loc; exp_env = env; exp_type = typ; _ } as e :
      expression) : imm_expression * anf_bind list =
  match exp_desc with
  ...
  | TExpBlock (fst :: rest) ->
      ...
      let rest_ans, rest_setup =
        transl_imm { e with exp_desc = TExpBlock rest }
      in
  ...
  | TExpLet (Nonrecursive, { vb_expr; vb_pat } :: rest, body) ->
      ...
      let body_ans, body_setup =
        transl_imm { e with exp_desc = TExpLet (Nonrecursive, rest, body) }
      in
        ...
        | TPatTuple args ->
            ( List.map
                (function
                  ...
                  | _ ->
                      failwith
                        "NYI: transl_imm: Destructuring in lambda argument")
                args,
              [] )
        | TPatVar (v, _) -> ([ v ], [])
        | TPatConstruct ({ txt = ident }, _, [])
  ...
  | TExpTuple args ->
      ...
      let new_args, new_setup = List.split (List.map transl_imm args) in
      ( Imm.id ~loc ~env tmp,
        List.concat new_setup @ [ BLet (tmp, Comp.tuple ~loc ~env new_args) ] )
  | TExpArray args ->
      let tmp = gensym "tup" in
      let new_args, new_setup = List.split (List.map transl_imm args) in
      ( Imm.id ~loc ~env tmp,
        List.concat new_setup @ [ BLet (tmp, Comp.array ~loc ~env new_args) ] )
  ...
  | TExpArraySet (arr, idx, arg) ->
      ...
      ( Imm.id ~loc ~env tmp,
        arr_setup @ idx_setup @ arg_setup
        @ [ BLet (tmp, Comp.array_set ~loc ~env idx_var arr_var arg_var) ] )
  | TExpRecord args ->
      let tmp = gensym "record" in
      let definitions =
        Array.to_list @@ Array.map (fun (desc, def) -> def) args
      in
      let definitions =
        List.map
          (function
            | Kept _ -> assert false | Overridden (name, def) -> (name, def))
          definitions
      in
      let new_args, new_setup =
        List.split
          (List.map
             (fun ({ txt = name; loc }, expr) ->
  ...
  | TExpRecordGet (expr, field, ld) ->
      ...
      ( Imm.id ~loc ~env tmp,
        setup
        @ [
            BLet (tmp, Comp.record_get ~loc ~env (Int32.of_int ld.lbl_pos) var);
          ] )
  | TExpMatch (exp, branches, _) ->
      ...
      let ans, setup =
        MatchCompiler.compile_result
          ...
          transl_anf_expression exp_ans
      in
      ( Imm.id ~loc ~env tmp,
        (exp_setup @ List.map (fun (n, e) -> BLet (n, e)) setup)
        @ [ BLet (tmp, ans) ] )
  | TExpConstruct _ -> failwith "NYI: transl_imm: Construct"

and bind_patts ?(exported = false) (pat : pattern) : Ident.t * anf_bind list =
  let postprocess_item cur acc =
    ...
    | Some (ident, (src, idx), extras) ->
        ...
        in
        let bind =
          if exported then BLetExport (Nonrecursive, [ (ident, access) ])
          else BLet (ident, access)
        in
        [ bind ] @ extras @ acc
  in
    ...
    | TPatTuple patts ->
        ...
        Some
          ( tmp,
            ...
            postprocess
            @@ List.mapi
                 (fun i pat -> anf_patts_pass_one tmp (TuplePatGet i) pat)
                 patts )
    | TPatRecord (fields, _) ->
...
and transl_comp_expression
  ...
  | TExpBlock (fst :: rest) ->
      let fst_ans, fst_setup = transl_comp_expression fst in
      let rest_ans, rest_setup =
        transl_comp_expression { e with exp_desc = TExpBlock rest }
      in
      (rest_ans, fst_setup @ [ BSeq fst_ans ] @ rest_setup)
  ...
  | TExpLet
      ...
      let body_ans, body_setup =
        transl_comp_expression
          { e with exp_desc = TExpLet (Nonrecursive, rest, body) }
      in
  ...
  let bind_constructor (cd_id, { cstr_name; cstr_tag; cstr_args }) =
    let rhs =
      ...
      | CstrBlock _ ->
          ...
          Comp.lambda ~loc ~env args
            (AExp.comp ~loc ~env (Comp.adt ~loc ~env imm_tytag imm_tag arg_ids))
...
let rec transl_anf_statement
    ...
    anf_bind list option * import_spec list =
  ...
  | TTopLet (export_flag, Nonrecursive, { vb_expr; vb_pat } :: rest) ->
      ...
      let rest_setup, rest_imp =
        transl_anf_statement
          { s with ttop_desc = TTopLet (export_flag, Nonrecursive, rest) }
      in
      ...
      let setup =
        ...
        | TPatVar (bind, _) | TPatAlias ({ pat_desc = TPatAny }, bind, _) ->
            if exported then [ BLetExport (Nonrecursive, [ (bind, exp_ans) ]) ]
            else [ BLet (bind, exp_ans) ]
        | TPatTuple _ | TPatRecord _ ->
        ...
        | _ ->
            failwith
              "NYI: transl_anf_statement: Non-record or non-tuple \
  ...
  | TTopData decls ->
      ...
      let bindings =
        ...
        @@ List.map
             ...
             decls
      in
      if List.length bindings > 0 then (Some bindings, []) else (None, [])
  | TTopForeign desc ->
      let arity = Ctype.arity desc.tvd_desc.ctyp_type in
      ( None,
        [
          Imp.wasm_func desc.tvd_id desc.tvd_mod.txt desc.tvd_name.txt
...
let transl_anf_module ({ statements; env; signature } : typed_program) :
    anf_program =
  ...
  let top_binds, imports =
    List.fold_left
      (fun (acc_bind, acc_imp) cur ->
        match cur with
        | None, lst -> (acc_bind, List.rev_append lst acc_imp)
        | Some b, lst ->
            (List.rev_append b acc_bind, List.rev_append lst acc_imp))
      prog_setup
