...
module BranchArg : Anf_mapper.MapArgument = struct
  ...
  let is_simple_case { anf_desc } =
    match anf_desc with AEComp _ -> true | _ -> false
    ...
    | AELet (_global, _recursive, binds, body) ->
        {
          a with
          anf_desc =
            AELet (_global, _recursive, binds, relinearize id global body cont);
        }
  ...
  let enter_anf_expression ({ anf_desc = desc } as a) =
    ...
    | AELet (global, Nonrecursive, binds, body)
      when has_optimizable_conditional binds ->
        ...
        List.fold_right
          (fun (name, ({ comp_desc } as comp)) cont ->
            match comp_desc with
