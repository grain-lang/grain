...
module MakeMap (Iter : MapArgument) = struct
  ...
  and map_comp_expression c =
    ...
    let d =
      ...
      | CRecord (ttag, elts) ->
          CRecord
            ( map_imm_expression ttag,
              List.map (fun (name, elt) -> (name, map_imm_expression elt)) elts
            )
      | CAdt (ttag, vtag, elts) ->
          CAdt
            ( map_imm_expression ttag,
      ...
      | CGetTupleItem (idx, tup) -> CGetTupleItem (idx, map_imm_expression tup)
      ...
      | CSwitch (c, branches) ->
          ...
          let branches =
            List.map
              (fun (tag, body) -> (tag, map_anf_expression body))
              branches
          in
          CSwitch (c, branches)
      | CApp (f, args) ->
