...
module ExpressionHash = struct
  type t = comp_expression_desc

  let compare_lists l1 l2 =
    List.length l1 == List.length l2
    && List.fold_left2
         (fun is_equal { imm_desc = exp1 } { imm_desc = exp2 } ->
           is_equal && exp1 = exp2)
         true l1 l2
  ...
  let equal i j =
    ...
    | CPrim1 (p1, { imm_desc = desc1 }), CPrim1 (p2, { imm_desc = desc2 }) ->
        desc1 = desc2 && p1 = p2
    | ( CPrim2 (p1, { imm_desc = x1 }, { imm_desc = y1 }),
        CPrim2 (p2, { imm_desc = x2 }, { imm_desc = y2 }) ) ->
        p1 = p2 && x1 = x2 && y1 = y2
    | CTuple exps1, CTuple exps2 -> compare_lists exps1 exps2
    | ( CAdt ({ imm_desc = ttag1 }, { imm_desc = vtag1 }, elts1),
        CAdt ({ imm_desc = ttag2 }, { imm_desc = vtag2 }, elts2) ) ->
  ...
  let hash i =
    ...
    | CPrim2 (p, { imm_desc = x }, { imm_desc = y }) ->
        Hashtbl.hash "CPrim2" lxor Hashtbl.hash p lxor Hashtbl.hash x
        lxor Hashtbl.hash y
    | CTuple exps ->
        Hashtbl.hash "CTuple"
        lxor List.fold_left
               (fun hash { imm_desc } -> hash lxor Hashtbl.hash imm_desc)
               0 exps
    | CAdt ({ imm_desc = ttag }, { imm_desc = vtag }, elts) ->
        Hashtbl.hash "CAdt" lxor Hashtbl.hash ttag lxor Hashtbl.hash vtag
        lxor List.fold_left
               (fun hash { imm_desc } -> hash lxor Hashtbl.hash imm_desc)
               0 elts
    | CApp ({ imm_desc = desc }, args) ->
        ...
        lxor List.fold_left
               (fun hash { imm_desc } -> hash lxor Hashtbl.hash imm_desc)
               (Hashtbl.hash desc) args
    | CAppBuiltin (_module, name, args) ->
        Hashtbl.hash "CAppBuiltin" lxor Hashtbl.hash _module
        lxor List.fold_left
               (fun hash { imm_desc } -> hash lxor Hashtbl.hash imm_desc)
               (Hashtbl.hash name) args
...
let create_rewrite_rule a b = rewrite_rules := Ident.add a b !rewrite_rules

let get_rewrite_rule id =
  try Ident.find_same id !rewrite_rules with Not_found -> id

let push_expression e id = ExpressionHashtbl.add known_expressions e id

let pop_expression e = ExpressionHashtbl.remove known_expressions e

let get_known_expression e = ExpressionHashtbl.find_opt known_expressions e
