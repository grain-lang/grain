...
let rec anf_free_vars_help env (a : anf_expression) =
  ...
  | AELet (_, recflag, binds, body) ->
      let with_names =
        List.fold_left (fun acc (id, _) -> Ident.Set.add id acc) env binds
      in
      let free_binds =
        match recflag with
        | Recursive ->
...
and comp_free_vars_help env (c : comp_expression) =
  ...
  | CSwitch (arg, branches) ->
      List.fold_left
        (fun acc (_, b) -> Ident.Set.union (anf_free_vars_help env b) acc)
  ...
  | CApp (fn, args) ->
      List.fold_left
        (fun acc a -> Ident.Set.union (imm_free_vars_help env a) acc)
  ...
  | CTuple args | CArray args | CAdt (_, _, args) ->
      List.fold_left
        (fun acc a -> Ident.Set.union (imm_free_vars_help env a) acc)
        Ident.Set.empty args
  | CArrayGet (arg1, arg2) ->
      List.fold_left
        (fun acc a -> Ident.Set.union (imm_free_vars_help env a) acc)
        Ident.Set.empty [ arg1; arg2 ]
  | CArraySet (arg1, arg2, arg3) ->
      List.fold_left
        (fun acc a -> Ident.Set.union (imm_free_vars_help env a) acc)
        Ident.Set.empty [ arg1; arg2; arg3 ]
  | CRecord (_, args) ->
      List.fold_left
        (fun acc (_, a) -> Ident.Set.union (imm_free_vars_help env a) acc)
        Ident.Set.empty args
  | CGetTupleItem (_, arg)
  | CGetAdtItem (_, arg)
  | CGetAdtTag arg
  ...
  | CSetTupleItem (_, arg1, arg2) ->
      Ident.Set.union
        (imm_free_vars_help env arg1)
        (imm_free_vars_help env arg2)
  | CInt32 _ | CInt64 _ | CString _ -> Ident.Set.empty
...
let rec anf_count_vars a =
  ...
  | AELet (_, recflag, binds, body) -> (
      let max_binds =
        List.fold_left max 0 @@ List.map (fun (_, c) -> comp_count_vars c) binds
      in
      match recflag with
...
and comp_count_vars c =
  ...
  | CSwitch (_, bs) ->
      List.fold_left max 0 @@ List.map (fun (_, b) -> anf_count_vars b) bs
...
module ClearLocationsArg : Anf_mapper.MapArgument = struct
  ...
  let leave_imm_expression i = { i with imm_loc = Location.dummy_loc }

  let leave_comp_expression c = { c with comp_loc = Location.dummy_loc }

  let leave_anf_expression a = { a with anf_loc = Location.dummy_loc }
end
