...
type analysis += TailCall of bool | TailRecursive of bool

let tail_callable_names = Ident_tbl.create 50

let push_tail_callable_name id = Ident_tbl.add tail_callable_names id true
...
let push_tail_recursive analysis = analysis := TailRecursive true :: !analysis

(* Indicate whether or not this expression contains a tail call that needs optimization *)
let rec analyze_comp_expression is_tail
  ...
  | CApp ({ imm_desc = ImmId id }, _) ->
      ...
      is_tail_callable id
  | CAppBuiltin _ | CApp _ ->
      if is_tail then push_tail_call analyses;
      false
  | CAssign _ | CTuple _ | CArray _ | CArrayGet _ | CArraySet _ | CRecord _
  | CAdt _ | CSetTupleItem _ | CGetTupleItem _ | CGetAdtItem _ | CGetAdtTag _
  | CGetRecordItem _ | CString _ | CInt32 _ | CInt64 _ | CPrim1 _ | CPrim2 _
  | CImmExpr _ ->
      false
