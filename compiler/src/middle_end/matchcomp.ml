...
module MatchTreeCompiler = struct
  ...
  let pattern_could_contain_binding patt =
    match patt.pat_desc with TPatConstant _ | TPatAny -> false | _ -> true
  ...
  let extract_bindings (patt : pattern) (expr : Anftree.comp_expression) :
      (Ident.t * Anftree.comp_expression) list =
    ...
    let rec extract_bindings patt expr =
      ...
      | TPatTuple args -> (
          ...
          let process_nested other_binds idx nested_pat =
            let this_binds =
              if pattern_could_contain_binding nested_pat then
                ...
                let arg_binds =
                  extract_bindings nested_pat (Comp.imm ~loc ~env tup_arg_imm)
                in
          ...
          match binds with [] -> [] | _ -> (tup_name, expr) :: binds )
      | TPatRecord (fields, _) -> (
          ...
          let process_nested other_binds (lid, ld, nested_pat) =
            let this_binds =
              if pattern_could_contain_binding nested_pat then
                let rec_field_name =
                  Ident.create @@ "match_bind_rec_field_"
                  ^ Identifier.last lid.txt
                in
                let rec_field_imm = Imm.id ~loc ~env rec_field_name in
                let field_binds =
                  extract_bindings nested_pat (Comp.imm ~loc ~env rec_field_imm)
                in
                match field_binds with
                | [] -> []
                | _ ->
                    ( rec_field_name,
                      Comp.record_get ~loc ~env (Int32.of_int ld.lbl_pos) rec_id
                    )
                    :: field_binds
              else []
            in
          ...
          match binds with [] -> [] | _ -> (rec_name, expr) :: binds )
      | TPatConstruct (_, _, args) -> (
          ...
          let process_nested other_binds idx nested_pat =
            let this_binds =
              if pattern_could_contain_binding nested_pat then
                ...
                let arg_binds =
                  extract_bindings nested_pat (Comp.imm ~loc ~env cstr_arg_imm)
                in
                match arg_binds with
                | [] -> []
                | _ ->
          ...
          in
          let binds = BatList.fold_lefti process_nested [] args in
          match binds with [] -> [] | _ -> (data_name, expr) :: binds )
  ...
  let rec compile_tree_help tree values =
    ...
    | Explode (_, (Leaf _ as inner)) | Explode (_, (Fail as inner)) ->
    ...
    | Explode (matrix_type, rest) ->
        ...
        in
        let new_values =
          List.map (fun (id, _) -> Imm.id id) bindings @ rest_values
        in
    ...
    | Switch (branches, default_tree) ->
        ...
        let switch_body_ans, switch_body_setup =
          List.fold_left
            (fun (body_ans, body_setup) (tag, tree) ->
  ...
  let compile_result { tree; branches } helpA expr :
      Anftree.comp_expression * (Ident.t * Anftree.comp_expression) list =
...
let rec pattern_always_matches patt =
  ...
  | TPatRecord (fields, _)
    when List.for_all (fun (_, _, p) -> pattern_always_matches p) fields ->
      true
  | _ -> false
...
let flatten_pattern size { pat_desc } =
  ...
  | TPatRecord (((_, { lbl_all }, _) :: _ as ps), _) ->
      let patterns =
        Array.init (Array.length lbl_all) (fun _ -> Parmatch.omega)
      in
      List.iter (fun (_, ld, pat) -> patterns.(ld.lbl_pos) <- pat) ps;
      Array.to_list patterns
  | TPatVar _ | TPatAny -> Parmatch.omegas size
  | _ ->
      failwith
...
let rec matrix_type = function
  ...
  | ({ pat_desc = TPatRecord ((_, ld, _) :: _, _) } :: _, _) :: _ ->
      RecordMatrix (Array.map (fun { lbl_pos } -> lbl_pos) ld.lbl_all)
...
let pattern_head_constructors patt = pattern_head_constructors_aux patt []
...
let matrix_head_constructors mtx =
  let rec help mtx acc =
    ...
    | (p :: _, mb) :: tl -> help tl (pattern_head_constructors_aux p acc)
  in
  help mtx []
...
let rec col_is_wildcard mtx col =
  match mtx with
  | [] -> true
  | (pats, _) :: _ when not (pattern_always_matches (List.nth pats col)) ->
      false
...
let rec rotate_if_needed mtx =
  if not (col_is_wildcard mtx 0) then (0, mtx)
  else
    match mtx with
    | [] -> (0, mtx)
    | _ ->
        ...
        let rec find_rotation n =
          if n >= width then raise Not_found
          else if not (col_is_wildcard mtx n) then (n, rotate_matrix mtx n)
          else find_rotation (n + 1)
        in
        find_rotation 1
  ...
  let rec specialized_rows row =
    ...
    | ({ pat_desc } as p) :: ptl -> (
        ...
        | _ when pattern_always_matches p ->
            let wildcards =
              BatList.init arity (fun _ -> { p with pat_desc = TPatAny })
            in
            [ wildcards @ ptl ]
        ...
        | _ -> [] )
    (* Specialization for non-constructors generates no rows. *)
  in

  List.fold_right
    (fun (row, mb) rem ->
      List.map (fun patts -> (patts, mb)) (specialized_rows row) @ rem)
    mtx []
...
let rec default_matrix mtx =
  ...
  List.fold_right
    (fun (row, mb) rem ->
      List.map (fun patts -> (patts, mb)) (default_rows row) @ rem)
    mtx []
...
let rec compile_matrix branches mtx =
  ...
  | (pats, (mb, i)) :: _ when List.for_all pattern_always_matches pats ->
      { tree = Leaf i; branches = (i, mb.mb_body, mb.mb_pat) :: branches }
  | _ when not (col_is_wildcard mtx 0) -> (
      ...
      | TupleMatrix arity ->
          let mtx =
            List.map
              (fun (row, mb) -> (flatten_pattern arity (List.hd row), mb))
              mtx
          in
          let result = compile_matrix branches mtx in
          { result with tree = Explode (matrix_type, result.tree) }
      | RecordMatrix labels ->
          let mtx =
            List.map
              (fun (row, mb) ->
      ...
      | ConstructorMatrix _ -> (
          ...
          | [] ->
              failwith
                "Internal error: compile_matrix: non-wildcard column returned \
                 no constructors"
          | _ ->
              let result, switch_branches =
                List.fold_left handle_constructor
                  ({ branches; tree = Fail }, [])
                  constructors
              in
              ...
              in
              { tree = Switch (switch_branches, default_tree); branches } ) )
  | _ ->
      ...
      let i, rotated =
        ...
        with Not_found ->
          failwith
            "Internal error: convert_match_branches: no non-wildcard column \
             found but not all patterns always match (should be impossible)"
      in
      let ({ tree; _ } as result) = compile_matrix branches rotated in
      { result with tree = Swap (i, tree) }

let convert_match_branches (match_branches : Typedtree.match_branch list) :
    conversion_result =
