...
module DAEArg: Anf_mapper.MapArgument = {
  ...
  let leave_anf_expression = ({anf_desc: desc} as a) =>
    switch (desc) {
    | [@implicit_arity] AELet(Global, _, _, _, _)
    | AESeq(_)
    | AEComp(_) => a
    | [@implicit_arity] AELet(g, Nonrecursive, mut_flag, [(bind, value)], body) =>
    ...
    | [@implicit_arity] AELet(g, r, m, binds, body) =>
      let new_binds =
        List.fold_right(
          ((id, v), tl) =>
            if (can_remove(id, v)) {
      ...
      switch (new_binds) {
      | [] => body
      | _ => {...a, anf_desc: [@implicit_arity] AELet(g, r, m, new_binds, body)}
      };
    };
};
