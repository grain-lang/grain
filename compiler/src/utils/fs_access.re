...
let modified_cache: Hashtbl.t(Filepath.t, float) = Hashtbl.create(16);
...
let exists_cache: Hashtbl.t(Filepath.t, bool) = Hashtbl.create(16);
...
let cache_flushers: ref(list((Filepath.t => unit, unit => unit))) =
  ref([]);
...
/** Recursive readdir */
let readdir = dir => {
  let dir = Filepath.to_fp(dir);
  let rec readdirExn = dir =>
    Fs.readDirExn(dir)
    |> List.fold_left(
         (results, filepath) => {
           Sys.is_directory(Filepath.to_string(Filepath.from_fp(filepath)))
             ? Array.append(results, readdirExn(filepath))
             : Array.append(results, [|filepath|])
         },
         [||],
       );
  readdirExn(dir) |> Array.map(Filepath.from_fp);
};
