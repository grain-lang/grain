type config_opt = Opt : ('a ref * 'a) -> config_opt

type saved_config_opt = SavedOpt : ('a ref * 'a) -> saved_config_opt
...
type config_spec = Spec : 'a Cmdliner.Arg.t * 'a ref -> config_spec

let opts : config_opt list ref = ref []

let specs : config_spec list ref = ref []

let internal_opt : 'a. 'a -> 'a ref =
 fun v ->
  let cur = ref v in
  opts := Opt (cur, v) :: !opts;
  cur
...
let opt :
      'a. ?docs:string -> ?docv:string -> ?doc:string -> ?env_docs:string ->
      ?env_doc:string -> ?env:string -> names:string list ->
      conv:'a Cmdliner.Arg.conv -> 'a -> 'a ref =
 fun ?docs ?docv ?doc ?env_docs ?env_doc ?env ~names ~conv:c v ->
  let cur = internal_opt v in
  specs :=
    Spec
      ( Cmdliner.Arg.(
          opt c v (arg_info ?docs ?docv ?doc ?env_docs ?env_doc ?env ~names)),
        cur )
    :: !specs;
  cur
...
let bool_flag :
    ?true_info:Cmdliner.Arg.info ->
    ?false_info:Cmdliner.Arg.info ->
    'a ->
    'a ref =
    ...
    Spec
      ( Cmdliner.Arg.(
          vflag default
            [
              ( not default,
                arg_info ?docs ?docv ?doc ?env_docs ?env_doc ?env ~names );
            ]),
        cur )
    :: !specs;
  cur
...
let save_config () =
  let single_save = function Opt (cur, _) -> SavedOpt (cur, !cur) in
  List.map single_save !opts

let restore_config =
  let single_restore = function SavedOpt (ptr, value) -> ptr := value in
  List.iter single_restore

let reset_config () =
  let single_reset = function Opt (cur, default) -> cur := default in
...
let with_cli_options (term : 'a) : 'a Cmdliner.Term.t =
  ...
  let open Term in
  let process_option acc = function
    | Spec (arg, box) ->
        const (fun a b ->
            box := a;
            b)
...
let option_conv (prsr, prntr) =
  ( (fun x -> match prsr x with `Ok a -> `Ok (Some a) | `Error a -> `Error a),
    fun ppf -> function
...
let include_dirs =
  opt ~names:[ "I" ]
    ...
    ~doc:"Extra library include directories" ~docv:"DIR" []

let stdlib_dir =
  opt ~names:[ "stdlib" ]
    ~conv:(option_conv Cmdliner.Arg.string)
    ~doc:"Path to the standard library (stdlib) directory" ~env:"GRAIN_STDLIB"
    None
...
let principal =
  toggle_flag ~names:[ "principal-types" ] ~doc:"Enable principal types" false

let recursive_types =
  toggle_flag ~names:[ "recursive-types" ] ~doc:"Enable recursive types" false

let strict_sequence =
  toggle_flag ~names:[ "strict-sequence" ] ~doc:"Enable strict sequencing" false

(* For now, leave this as true *)
let safe_string = ref true

let parser_debug_level =
  opt ~names:[ "parser-debug-level" ] ~conv:Cmdliner.Arg.int
    ~doc:"Debugging level for parser output" 0

let debug =
  toggle_flag ~names:[ "g" ] ~doc:"Compile with debugging information" false

let verbose =
  toggle_flag ~names:[ "cdebug" ] ~doc:"Print internal debug messages" false

let sexp_locs_enabled =
  toggle_flag ~names:[ "hide-locs" ]
    ~doc:
      "Hide locations from intermediate trees. Only has an effect with \
       `--cdebug'."
    true

let unsound_optimizations =
  toggle_flag ~names:[ "Ounsound" ]
    ~doc:"Compile with optimizations which may remove runtime errors" false
