...
and strengthen_sig ~aliasable env sg p pos =
  ...
  | (TSigValue (_, desc) as sigelt) :: rem ->
      let nextpos =
        match desc.val_kind with TValPrim _ -> pos | _ -> pos + 1
      in
...
let nondep_supertype env mid mty =
  ...
  and nondep_sig env va = function
    ...
    | item :: rem -> (
        ...
        | TSigValue (id, d) ->
            TSigValue
              (id, { d with val_type = Ctype.nondep_type env mid d.val_type })
            :: rem'
        | TSigType (id, d, rs) ->
        ...
        | TSigModule (id, md, rs) ->
            TSigModule
              (id, { md with md_type = nondep_mty env va md.md_type }, rs)
            :: rem'
        | TSigModType (id, d) -> (
...
let enrich_typedecl env p id decl =
  ...
  | None -> (
      try
        ...
        if decl.type_arity <> orig_decl.type_arity then decl
        else
          let orig_ty =
            Ctype.reify_univars
              (Btype.newgenty
...
and contains_type_item env = function
  ...
  | TSigModType _ (*| Sig_typext (_, {ext_args = Cstr_record _}, _)*) ->
...
let rec get_prefixes = function
  ...
  | PExternal (p, _, _) (*| Papply (p, _)*) -> PathSet.add p (get_prefixes p)
...
let rec rollback_path subst p =
  ...
  with Not_found -> (
    ...
    | PExternal (p1, s, n) ->
        ...
        else rollback_path subst (PExternal (p1', s, n)) )
...
let rec remove_aliases_mty env excl mty =
  ...
  | TModAlias _ ->
      ...
      if mty' = mty then mty else remove_aliases_mty env excl mty'
  | mty -> mty
...
let scrape_for_type_of ~remove_aliases env mty =
  ...
  else scrape_for_type_of env mty
