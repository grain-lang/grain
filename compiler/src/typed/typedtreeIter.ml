...
module MakeIterator (Iter : IteratorArgument) : sig
  ...
  and iter_core_type ct =
    ...
    | TTyArrow (args, ret) ->
        ...
        iter_core_type ret
    | TTyConstr (_, _, args) | TTyTuple args -> List.iter iter_core_type args
  ...
  and iter_toplevel_stmt stmt =
    ...
    | TTopForeign _ | TTopImport _ | TTopExport _ -> ()
  ...
  and iter_toplevel_stmts stmts =
    List.iter
      (fun cur ->
        ...
        | TTopForeign _ | TTopImport _ | TTopExport _ | TTopExpr _ | TTopLet _
          ->
  ...
  and iter_pattern pat =
    ...
    List.iter
      (fun (cstr, _) ->
        match cstr with TPatConstraint ct -> iter_core_type ct)
      pat.pat_extra;
    ( match pat.pat_desc with
    | TPatAny | TPatVar _ | TPatConstant _ -> ()
    | TPatAlias (p1, _, _) -> iter_pattern p1
    | TPatConstruct (_, _, args) | TPatTuple args -> List.iter iter_pattern args
    | TPatRecord (fields, _) ->
        List.iter (fun (_, _, pat) -> iter_pattern pat) fields
    | TPatOr (p1, p2) ->
        iter_pattern p1;
        iter_pattern p2 );
  ...
  and iter_expression ({ exp_desc; exp_extra } as exp) =
    ...
    List.iter
      (fun (cstr, _) ->
        match cstr with TExpConstraint ct -> iter_core_type ct)
      exp_extra;
    ( match exp_desc with
    | TExpNull | TExpIdent _ | TExpConstant _ -> ()
    ...
    | TExpRecord args ->
        Array.iter
          (function _, Overridden (_, expr) -> iter_expression expr | _ -> ())
          args
    | TExpRecordGet (expr, _, _) -> iter_expression expr
    | TExpTuple args
    | TExpArray args
    | TExpBlock args
    | TExpConstruct (_, _, args) ->
        List.iter iter_expression args
    | TExpArrayGet (a1, a2) ->
        iter_expression a1;
        iter_expression a2
