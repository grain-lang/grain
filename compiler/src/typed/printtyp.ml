...
let rec safe_commu_repr v = function
  ...
  | TComLink r ->
      if List.memq r v then "TComLink loop" else safe_commu_repr (r :: v) !r

let rec safe_repr v = function
  | { desc = TTyLink t } when not (List.memq t v) -> safe_repr (t :: v) t
  | t -> t
...
and raw_type_desc ppf = function
  ...
  | TTyArrow (t1, t2, c) ->
      fprintf ppf "@[<hov1>TTyArrow(@,%a,@,%a,@,%s)@]" raw_type_list t1 raw_type
        t2 (safe_commu_repr [] c)
  | TTyTuple tl -> fprintf ppf "@[<1>TTyTuple@,%a@]" raw_type_list tl
  | TTyRecord tl ->
      fprintf ppf "@[<1>TTyRecord@,%a@]"
        (raw_list (fun ppf (name, arg) -> fprintf ppf "%s=%a" name raw_type arg))
        tl
  | TTyConstr (p, tl, abbrev) ->
      fprintf ppf "@[<hov1>TTyConstr(@,%a,@,%a,@,%a)@]" path p raw_type_list tl
  ...
  | TTyPoly (t, tl) ->
      fprintf ppf "@[<hov1>TTyPoly(@,%a,@,%a)@]" raw_type t raw_type_list tl
...
let aliasable ty =
  ...
  | TTyConstr (p, _, _) -> not (is_nth (snd (best_type_path p)))
  | _ -> true
...
let rec mark_loops_rec visited ty =
  ...
  if List.memq px visited && aliasable ty then add_alias px
    ...
    | TTyRecord tyl ->
        List.iter (fun (_, arg) -> (mark_loops_rec visited) arg) tyl
    | TTyConstr (p, tyl, _) ->
    ...
    let pr_typ () =
      ...
      | TTyArrow (ty1, ty2, _) ->
          let pr_arrow ty1 ty2 =
            let t1 = tree_of_typlist sch ty1 in
      ...
      | TTyRecord tyl ->
          Otyp_record
            (List.map
               (fun (name, arg) -> (name, false, tree_of_typexp sch arg))
               tyl)
...
let rec tree_of_type_decl id decl =
  ...
  let type_param = function Otyp_var (_, id) -> id | _ -> "?" in
  let type_defined decl =
    let vari = List.map (fun ty -> (true, true)) decl.type_params in
    ( Ident.name id,
      List.map2
...
let filter_rem_sig item rem = match (item, rem) with _ -> ([], rem)
...
let rec tree_of_modtype ?(ellipsis = false) = function
  ...
  | TModSignature sg ->
      Omty_signature
        (if ellipsis then [ Osig_ellipsis ] else tree_of_signature sg)
...
let rec trace fst txt ppf = function
  | (t1, t1') :: (t2, t2') :: rem ->
      ...
      fprintf ppf "@[Type@;<1 2>%a@ %s@;<1 2>%a@] %a" (type_expansion t1) t1'
        txt (type_expansion t2) t2' (trace false txt) rem
  | _ -> ()
...
let hide_variant_name t = match repr t with _ -> t
...
let may_prepare_expansion compact (t, t') =
  match (repr t').desc with _ -> prepare_expansion (t, t')
...
let explanation env unif t3 t4 : (Format.formatter -> unit) option =
  ...
  | TTyConstr (p, _, _), TTyVar _ when unif && t4.level < Path.binding_time p ->
      Some
        (fun ppf ->
          fprintf ppf
            "@,@[The type constructor@;<1 2>%a@ would escape its scope@]" path p)
  | TTyVar _, TTyConstr (p, _, _) when unif && t3.level < Path.binding_time p ->
      Some
        (fun ppf ->
          fprintf ppf
            "@,@[The type constructor@;<1 2>%a@ would escape its scope@]" path p)
        ...
        (fun ppf ->
          ...
          if occur_in Env.empty t t' then
            ...
            fprintf ppf "@,@[<hov>This instance of %a is ambiguous:@ %s@]"
              type_expr t' "it would escape the scope of its equation")
  | _ -> None

let rec mismatch env unif = function
  | (_, t) :: (_, t') :: rem -> (
      match mismatch env unif rem with
      | Some _ as m -> m
      | None -> explanation env unif t t' )
  | [] -> None
  | _ -> assert false

let explain mis ppf = match mis with None -> () | Some explain -> explain ppf
...
let unification_error env unif tr txt1 ppf txt2 ty_expect_explanation =
  ...
  | t1 :: t2 :: tr -> (
      try
        ...
        fprintf ppf "@[<v>@[%t@;<1 2>%a@ %t@;<1 2>%a%t@]%a%t@]" txt1
...
let report_unification_error ppf env ?(unif = true) tr
    ?(type_expected_explanation = fun _ -> ()) txt1 txt2 =
  wrap_printing_env ~error:true env (fun () ->
      unification_error env unif tr txt1 ppf txt2 type_expected_explanation)
...
let report_ambiguous_type_error ppf env (tp0, tp0') tpl txt1 txt2 txt3 =
  wrap_printing_env ~error:true env (fun () ->
      ...
      | [] -> assert false
      | [ (tp, tp') ] ->
          fprintf ppf "@[%t@;<1 2>%a@ %t@;<1 2>%a@]" txt1
            (type_path_expansion tp) tp' txt3 (type_path_expansion tp0) tp0'
      | _ ->
