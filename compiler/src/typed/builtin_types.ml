...
let decl_abstr_imm path = {(decl_abstr path) with type_immediate = true}
...
let decl_exception =
  decl_create {(decl_abstr path_exception) with
    type_kind = TDataVariant([])}
let decl_bool =
  decl_create {(decl_abstr path_bool) with
    type_kind = TDataVariant([cstr ident_false []; cstr ident_true []]);
    type_immediate = true}
and decl_void =
  decl_create {(decl_abstr path_void) with
    type_kind = TDataVariant([cstr ident_void_cstr []]);
    type_immediate = true}
and decl_box =
  let tvar = newgenvar() in
  decl_create {(decl_abstr path_box) with
    type_params = [tvar];
    type_arity = 1}
and decl_array =
  let tvar = newgenvar() in
  decl_create {(decl_abstr path_array) with
    type_params = [tvar];
    type_arity = 1}
