...
let out_constr = ref (fun _ -> failwith "Oprint.out_constr")
...
and print_out_sig_item ppf =
  ...
  | Osig_typext (ext, Oext_exception) ->
      fprintf ppf "@[<2>exception %a@]" print_out_constr (ext.oext_name, ext.oext_args, None)
  | Osig_typext (ext, _es) ->
...
and print_out_extension_constructor ppf ext =
  ...
  fprintf ppf "@[<hv 2>type %t +=%s@;<1 2>%a@]"
    print_extended_type
    ""
    print_out_constr (ext.oext_name, ext.oext_args, None)
