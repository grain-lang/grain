...
(* see cmi *)
type module_coercion = unit option
...
let type_declarations ~loc env ~mark ?(old_env = env) cxt subst id decl1 decl2 =
  ...
  let err =
    Includecore.type_declarations ~loc env ~mark (Ident.name id) decl1 id decl2
  in
  if err <> [] then
...
let is_runtime_component = function
  ...
  | TSigType (_, _, _)
  | TSigModType (_, _) (*| Sig_class_type(_,_,_)*) ->
      false
  | TSigValue (_, _)
  (*| Sig_typext(_,_,_)*)
  | TSigModule (_, _, _) (*| Sig_class(_, _,_)*) ->
      true
...
let rec modtypes ~loc env ~mark cxt subst mty1 mty2 =
  try try_modtypes ~loc env ~mark cxt subst mty1 mty2 with
  | Dont_match ->
      raise
...
and try_modtypes2 ~loc env ~mark cxt mty1 mty2 =
  ...
  | _, TModIdent p2 when may_expand_module_path env p2 ->
      try_modtypes ~loc env ~mark cxt Subst.identity mty1
...
and signatures ~loc env ~mark cxt subst sig1 sig2 =
  ...
  let new_env = Env.add_signature sig1 env (*(Env.in_signature true env)*) in
  (* Keep ids for module aliases *)
  let id_pos_list, _ =
    List.fold_left
      (fun (l, pos) -> function
        | TSigModule (id, _, _) ->
            ((id, pos, None (*Tcoerce_none*)) :: l, pos + 1)
...
and modtype_infos ~loc env ~mark cxt subst id info1 info2 =
  ...
  try
    ...
    | None, None -> ()
    | Some _, None -> ()
    | Some mty1, Some mty2 -> check_modtype_equiv ~loc env ~mark cxt' mty1 mty2
    | None, Some mty2 ->
        check_modtype_equiv ~loc env ~mark cxt' (TModIdent (PIdent id)) mty2
  with Error reasons ->
...
and check_modtype_equiv ~loc env ~mark cxt mty1 mty2 =
  ...
  with
  | None, None (*(Tcoerce_none, Tcoerce_none)*) -> ()
  | _c1, _c2 ->
...
let compunit env ?(mark = Mark_both) impl_name impl_sig intf_name intf_sig =
  try
    signatures
      ...
      env ~mark [] Subst.identity impl_sig intf_sig
  with Error reasons ->
    raise
      (Error
...
let signatures env ?(mark = Mark_both) sig1 sig2 =
  signatures ~loc:Location.dummy_loc env ~mark [] Subst.identity sig1 sig2
...
let include_err ppf = function
  ...
  | Type_declarations (id, d1, d2, errs) ->
      fprintf ppf "@[<v>@[<hv>%s:@;<1 2>%a@ %s@;<1 2>%a@]%a%a@]"
        ...
        (Includecore.report_type_mismatch "the first" "the second"
           "declaration")
        errs
...
let path_of_context = function
  | Module id :: rem ->
      let rec subm path = function
        | [] -> path
        | Module id :: rem -> subm (PExternal (path, Ident.name id, -1)) rem
        | _ -> assert false
      in
      subm (PIdent id) rem
  | _ -> assert false
...
let context ppf cxt =
  if cxt = [] then ()
  else if List.for_all (function Module _ -> true | _ -> false) cxt then
