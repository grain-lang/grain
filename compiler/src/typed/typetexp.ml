...
type error =
  ...
  | Ill_typed_functor_application of
      Identifier.t * Identifier.t * Includemod.error list option
...
let lookup_module ?(load = false) env loc lid filepath =
  find_component
    (fun ?mark lid env -> Env.lookup_module ~load ~loc ?mark lid filepath env)
...
let find_modtype env loc lid =
  let ((path, decl) as r) =
    find_component
      (fun ?(mark = false) a b -> Env.lookup_modtype ~loc ~mark a b)
...
let rec transl_type env policy styp = transl_type_aux env policy styp
...
and transl_type_aux env policy styp =
  ...
  | PTyPoly (vars, st) ->
      ...
      let ty_list =
        List.fold_left
          (fun tyl (name, ty1) ->
            let v = ty1 (* would be a no-op right now *)
                        (*Btype.proxy ty1*) in
...
let fold_persistent fold4 f =
  fold4 (fun name path _descr acc ->
      if Ident.persistent (Path.head path) then f name acc else acc)
...
let report_error env ppf = function
  ...
  | Unbound_label _ | Unbound_class _ | Unbound_cltype _ ->
  ...
  | Ill_typed_functor_application (flid, mlid, details) -> (
      match details with
      | None ->
          fprintf ppf "@[Ill-typed functor application %a(%a)@]" identifier flid
            identifier mlid
      | Some inclusion_error ->
          fprintf ppf "@[The type of %a does not match %a's parameter@\n%a@]"
            identifier mlid identifier flid Includemod.report_error
            inclusion_error )
  | Illegal_reference_to_recursive_module ->
  ...
  | Wrong_use_of_module (lid, details) -> (
      ...
      | `Generative_used_as_applicative ->
          fprintf ppf
            "@[The functor %a is generative,@ it@ cannot@ be@ applied@ in@ \
             type@ expressions@]"
            identifier lid )
  ...
  | Opened_object nm ->
      fprintf ppf "Illegal open object type%a"
        (fun ppf -> function Some p -> fprintf ppf "@ %a" path p
          | None -> fprintf ppf "")
        nm
  | Not_an_object ty ->
      Printtyp.reset_and_mark_loops ty;
