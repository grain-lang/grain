...
let proper_abbrevs path tl abbrev =
  if tl <> [] || !Grain_utils.Config.principal || is_object_type path then
    abbrev
  else simple_abbrevs
...
let is_datatype decl =
  ...
  | TDataRecord _ | TDataVariant _ -> true
  | TDataAbstract -> false
        ...
        List.iter
          (fun { cd_args; cd_res; _ } ->
            ...
            | None -> (
                ...
                | TConstrSingleton -> () ))
          v );
    (match decl.type_manifest with None -> () | Some ty -> closed_type ty);
...
let rec normalize_package_path env p = failwith "NYI: normalize_package_path"
...
let rec copy ?env ?partial ?keep_names ty =
  ...
  | _ ->
      if ty.level <> generic_level && partial = None then ty
        ...
        in
        if forget <> generic_level then newty2 forget (TTyVar None)
        else
          let desc = ty.desc in
          save_desc ty desc;
          let t = newvar () in
...
let map_kind f = function
  ...
  | TDataRecord fields ->
      TDataRecord
        (List.map
           (fun field -> { field with rf_type = f field.rf_type })
           fields)
  | TDataVariant cl ->
      TDataVariant
        (List.map
...
let rec copy_sep fixed free bound visited ty =
  ...
  if TypeSet.is_empty univars then (
    ...
    with Not_found ->
      ...
      let visited =
        match ty.desc with
        | TTyArrow _ | TTyTuple _ | TTyConstr _ -> (ty, (t, bound)) :: visited
        | _ -> visited
      in
...
let apply env params body args =
  try subst env generic_level Public (ref TMemNil) None params args body
...
let rec extract_concrete_typedecl env ty =
  ...
  | TTyConstr (p, _, _) ->
      let decl = Env.find_type p env in
      if decl.type_kind <> TDataAbstract then (p, p, decl)
      else
        let ty =
...
let univars_escape env univar_pairs vl ty =
  ...
  let rec occur t =
    ...
    if TypeSet.mem t !visited then ()
      ...
      | TTyUniVar _ -> if TypeSet.mem t family then raise Occur
      | TTyConstr (_, [], _) -> ()
      | TTyConstr (p, tl, _) -> (
          try
            (*let td = Env.find_type p env in*)
      ...
      | _ -> (
          ...
          if t1' == t2' then ()
            ...
            with Not_found -> (
              ...
              | TTyTuple tl1, TTyTuple tl2 -> mcomp_list type_pairs env tl1 tl2
              ...
              | TTyConstr (p, _, _), _ | _, TTyConstr (p, _, _) -> (
                  try
                    ...
                    if non_aliasable p decl || is_datatype decl then
                      raise (Unify [])
                  with Not_found -> () )
              | TTyPoly (t1, []), TTyPoly (t2, []) -> mcomp type_pairs env t1 t2
              | TTyPoly (t1, tl1), TTyPoly (t2, tl2) ->
                  enter_poly env univar_pairs t1 tl1 t2 tl2
                    (mcomp type_pairs env)
              | TTyUniVar _, TTyUniVar _ -> unify_univar t1' t2' !univar_pairs
              | _, _ -> raise (Unify []) ) )
...
and mcomp_type_decl type_pairs env p1 p2 tl1 tl2 =
  try
    ...
    if compatible_paths p1 p2 then
      ...
      failwith "Internal error: mcomp_type_decl > compatible_paths"
      ...
      | TDataAbstract, TDataAbstract -> ()
      | TDataAbstract, _ when not (non_aliasable p1 decl) -> ()
      | _, TDataAbstract when not (non_aliasable p2 decl') -> ()
      | _ -> raise (Unify [])
  with Not_found -> ()
...
let rec concat_longident lid1 =
  ...
  | IdentExternal (lid2, s) -> IdentExternal (concat_longident lid1 lid2, s)
...
let unify_package env unify_list lv1 p1 n1 tl1 lv2 p2 n2 tl2 =
  ...
  and ntl1 = complete_type_list env n2 lv1 (TModIdent p1) n1 tl1 in
  ...
  if t1 == t2 then ()
    ...
    if unify_eq t1 t2 then ()
      ...
      try
        type_changed := true;
        match (t1.desc, t2.desc) with
        | TTyVar _, TTyConstr _ when deep_occur t1 t2 -> unify2 env t1 t2
        | TTyConstr _, TTyVar _ when deep_occur t2 t1 -> unify2 env t1 t2
                ...
                (fun (*i*) (t1, t2) ->
                  ...
                  set_mode_pattern ~generate:false ~injective:false (fun () ->
                      ...
                      with Unify _ ->
                        ...
                        reify env t2))
                (*inj*) (List.combine tl1 tl2)
        | (TTyConstr (_, _, _), _ | _, TTyConstr (_, _, _))
          when !umode = Pattern ->
            reify env t1';
            reify env t2';
...
let unify_var env t1 t2 =
  ...
  if t1 == t2 then ()
    ...
    match (t1.desc, t2.desc) with
    | TTyVar _, TTyConstr _ when deep_occur t1 t2 -> unify (ref env) t1 t2
    | TTyVar _, _ -> (
        try
          occur env t1 t2;
          update_level env t1.level t2;
...
let rec rigidify_rec vars ty =
  ...
  if ty.level >= lowest_level then (
    ty.level <- pivot_level - ty.level;
    match ty.desc with
    | TTyVar _ -> if not (List.memq ty !vars) then vars := ty :: !vars
...
let normalize_subst subst =
  if
    List.exists
      (function
        | { desc = TTyLink _ }, _ | _, { desc = TTyLink _ } -> true | _ -> false)
      !subst
...
let rec arity ty =
  match (repr ty).desc with TTyArrow (tl, _, _) -> List.length tl | _ -> 0
...
let nondep_type_decl env mid id is_covariant decl =
  try
    ...
    let tk =
      ...
      with Not_found when is_covariant -> TDataAbstract
    and tm =
      try
