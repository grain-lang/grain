...
let constructor_existentials cd_args cd_res =
  let tyl = match cd_args with TConstrSingleton -> [] | TConstrTuple l -> l in
  let existentials =
    match cd_res with
    | None -> []
    | Some type_ret ->
...
let constructor_descrs ty_path decl cstrs =
  ...
  let describe_constructor idx { cd_id; cd_args; cd_res; cd_loc } =
    let ty_res = match cd_res with Some ty_res' -> ty_res' | None -> ty_res in
    let tag =
      match cd_args with
...
let rec find_constr tag num_const num_nonconst = function
  ...
  | ({ cd_args = TConstrSingleton; _ } as c) :: rem ->
      if tag = CstrConstant num_const then c
      else find_constr tag (num_const + 1) num_nonconst rem
  | c :: rem ->
      if tag = CstrBlock num_nonconst || tag = CstrUnboxed then c
