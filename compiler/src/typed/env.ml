...
module PathMap = struct
  ...
  let sexp_of_t conv m =
    ...
    let pairs =
      List.map
        (fun (key, v) -> Sexp.List [ sexp_of_path key; conv v ])
        (bindings m)
    in
    Sexp.List pairs
...
and module_components = {
  ...
  comps :
    ( t * Subst.t * Path.t * Types.module_type,
      module_component_components option )
    EnvLazy.t;
}
...
let copy_local ~from env =
  { env with local_constraints = from.local_constraints }

let same_constr = ref (fun _ _ _ -> assert false)

type can_load_modules = Can_load_modules | Cannot_load_modules of EnvLazy.log
...
let get_components c =
  match get_components_opt c with None -> empty_structure | Some c -> c
...
let unit_to_file = (Hashtbl.create 17 : (string, string) Hashtbl.t)

let compilation_in_progress =
  (Hashtbl.create 17 : (string, string Location.loc) Hashtbl.t)
...
let mark_in_progress ~loc unit_name sourcefile =
  ...
  Hashtbl.add compilation_in_progress sourcefile
...
type module_location_result =
  ...
  | WasmModule of string

(* Compiled object *)

let compile_module_dependency =
  ref (fun filename output_file ->
...
let get_up_to_date ~loc unit_name = function
  ...
  | GrainModule (srcpath, Some objpath) when file_older srcpath objpath ->
      objpath
...
let locate_module path unit_name =
  ...
  | objpath, dir, basename, ext -> (
      match find_ext_in_dir dir basename grain_src_exts with
...
let locate_module_file ~loc path unit_name =
  ...
  let located =
    ...
    with Not_found -> error (No_module_file (unit_name, None))
  in
...
module Persistent_signature = struct
  ...
  let load =
    ref (fun ?(loc = Location.dummy_loc) ~unit_name ->
        match
          locate_module_file ~loc
...
let acknowledge_pers_struct check modname { Persistent_signature.filename; cmi }
  ...
  let comps =
    !components_of_module' ~deprecated:None ~loc:Location.dummy_loc empty
      Subst.identity
      ...
      | Can_load_modules ->
          let ps =
            let filepath =
              ...
              with Option.No_value -> failwith "No file path specified"
            in
...
let check_pers_struct ~loc name filename =
  try ignore (find_pers_struct ~loc false name filename) with
  | Not_found ->
  ...
  | Error err ->
      let msg =
        ...
        | Illegal_renaming (name, ps_name, filename) ->
            Format.asprintf
              " %a@ contains the compiled interface for @ %s when %s was \
               expected"
...
let find_pers_struct name filename = find_pers_struct true name filename
...
let rec find_module_descr path filename env =
  ...
  | PIdent id -> (
      ...
      with Not_found ->
        ...
        if
          Ident.persistent id
          && not (Option.default (Ident.name id) filename = unit_source)
        then
...
let find_type_full = find (fun env -> env.types) (fun sc -> sc.comp_types)
...
(*Printf.eprintf "Marking value %s used\n" name*)
...
and lookup_module ?loc ~load ~mark id filename env : Path.t =
  ...
  | Identifier.IdentName s -> (
      ...
      with Not_found ->
        ...
        if (*!Grain_utils.Config.transparent_modules &&*) not load then
          check_pers_struct ~loc s filename
        else ignore (find_pers_struct ~loc s filename);
        p )
...
let lookup_tycomptbl ~mark proj1 proj2 id env =
  ...
  | IdentExternal (m, n) ->
      let p, desc = lookup_module_descr ~mark id env in
      let comps =
        try Tbl.find n (proj2 (get_components desc)) with Not_found -> []
      in
...
let lookup_all_constructors ~mark id env =
  lookup_tycomptbl ~mark
    (fun x -> x.constructors)
    (fun x -> x.comp_constrs)
    id env
...
let lookup_all_constructors ?(mark = true) lid env =
  ...
  with
  | Not_found
  when match lid with Identifier.IdentName _ -> true | _ -> false
  ->
    []
...
let lookup_modtype ?loc ?(mark = true) lid env = lookup_modtype ~mark lid env
...
let rec scrape_alias_for_visit env mty = match mty with _ -> true
...
let find_all_comps proj s (p, mcomps) =
  let comps = get_components mcomps in
  try
    let c, n = Tbl.find s (proj comps) in
    [ (PExternal (p, s, n), c) ]
  with Not_found -> []
...
let rec prefix_idents root pos sub = function
  ...
  | TSigValue (id, decl) :: rem ->
      ...
      let nextpos =
        match decl.val_kind with TValPrim _ -> pos | _ -> pos + 1
      in
  ...
  | TSigModType (id, _) :: rem ->
      let p = PExternal (root, Ident.name id, nopos) in
      let pl, final_sub =
        prefix_idents root pos (Subst.add_modtype id (TModIdent p) sub) rem
      in
      (p :: pl, final_sub)
...
and components_of_module_maker (env, sub, path, mty) =
  ...
  | TModSignature sg ->
      ...
      let pl, sub =
        match (mty, path) with
        | TModAlias path, _ | TModIdent path, _ | TModSignature _, path ->
            prefix_idents path sub sg
      in
      let env = ref env in
      let pos = ref 0 in
      List.iter2
        (fun item path ->
          ...
          | TSigValue (id, decl) -> (
              ...
              match decl.val_kind with TValPrim _ -> () | _ -> incr pos )
          | TSigType (id, decl, _) ->
              ...
              List.iter
                (fun (id, desc) ->
                  let val_type =
                    match desc.cstr_args with
                    | [] -> desc.cstr_res
                    | args ->
                        Btype.newgenty (TTyArrow (args, desc.cstr_res, TComOk))
                  in
                  let val_type =
                    match desc.cstr_existentials with
                    | [] -> val_type
                    | existentials ->
          ...
          | TSigModule (id, md, _) ->
              ...
              let comps =
                components_of_module ~deprecated:None ~loc:md.md_loc !env sub
                  path md.md_type
              in
              c.comp_components <-
...
and store_type ~check id info env =
      ...
      end;*)
  {
    ...
    values =
      List.fold_left
        (fun acc (id, val_desc) -> IdTbl.add id val_desc acc)
        env.values val_descrs;
    summary =
      List.fold_left
        (fun acc (id, val_desc) -> Env_value (acc, id, val_desc))
...
and store_module ~check id md env =
        ...
        module_declarations;***)
  {
    ...
    components =
      IdTbl.add id
        (components_of_module ~deprecated:None ~loc:md.md_loc env Subst.identity
...
let add_components ?filter_modules ?filter_components slot root env0 comps =
  ...
  let constructors =
    add_l
      (fun x -> `Constructor x)
      filtered_components.comp_constrs env0.constructors
  in
  ...
  let modtypes =
    add
      (fun x -> `Module_type x)
      filtered_components.comp_modtypes env0.modtypes
  in
  let components =
    filter_and_add
      (fun x -> `Component x)
      filtered_components.comp_components env0.components
  in

  let modules =
    filter_and_add
      (fun x -> `Module x)
      filtered_components.comp_modules env0.modules
  in
...
let same_filepath unit1 unit2 = resolve_unit unit1 = resolve_unit unit2
...
let add_module_signature ?(internal = false) mod_name
    (mod_ : Parsetree.import_declaration) env0 =
  let name =
    match mod_name with
  ...
  in
  let mod_ident =
    if internal then Ident.create name else Ident.create_persistent mod_alias
  in
...
let open_pers_signature name filepath env =
  match
    open_signature None (PIdent (Ident.create_persistent name)) filepath env
  with
  | Some env -> env
  | None -> assert false
...
let open_signature_of_initially_opened_module ?(loc = Location.dummy_loc) root
    env =
...
let check_imports found all where =
  List.iter
    (fun comp ->
      if List.mem comp found then ()
      else
        let { loc; txt } = comp in
        error
          (Value_not_found_in_module (loc, Identifier.string_of_ident txt, where)))
    all

let open_signature ?(used_slot = ref false) ?(toplevel = false) root mod_name
    (mod_ : Parsetree.import_declaration) env =
  ...
  | PImportValues values ->
      ...
      in
      let root =
        match check_opened mod_ env with
        | Some path -> path
        | None -> assert false
      in
      let new_env =
        open_signature ~filter_components None root (Some mod_.pimp_path.txt)
          env
      in
      check_imports !imported
        (List.map (fun (value, _) -> value) values)
        mod_.pimp_path.txt;
      new_env
...
let crc_of_unit name filename =
  ...
  match crco with None -> assert false | Some crc -> crc
...
let build_signature_with_imports ?deprecated sg modname filename imports =
  ...
  let flags =
    List.concat
      [
        ( if !Grain_utils.Config.recursive_types then [ Cmi_format.Rectypes ]
        else [] );
        (*if !Grain_utils.Config.opaque then [Cmi_format.Opaque] else [];*)
      ]
  in
  try
    ...
    in
    let full_cmi =
      Cmi_format.build_full_cmi ~name:modname ~sign:sg ~crcs:imports ~flags
    in
    let crc =
      match full_cmi.cmi_crcs with
      | (_, Some crc) :: _ -> crc
    ...
    (* Enter signature in persistent table so that imported_unit()
       will also return its crc *)
    let comps =
      components_of_module ~deprecated ~loc:Location.dummy_loc empty
        Subst.identity
...
let format_dependency_chain ppf (depchain : dependency_chain) =
  let print_single { txt; loc } =
    ...
    if loc = Location.dummy_loc then fprintf ppf "<unknown>"
    else fprintf ppf "%a" Location.print_compact loc;
    fprintf ppf "@]"
  in
...
let report_error ppf = function
  | Illegal_renaming (modname, ps_name, filename) ->
      fprintf ppf
        "Wrong file naming: %a@ contains the compiled interface for @ %s when \
         %s was expected"
  ...
  | Depend_on_unsafe_string_unit (import, export) ->
      fprintf ppf
        ...
        export import
        "This compiler has been configured in strict safe-string mode \
         (-force-safe-string)"
  ...
  | Unbound_label (_, label) ->
      fprintf ppf
        "Unbound record label %s. Perhaps you need to import its type or write \
         a type definition?"
        label
  ...
  | Cyclic_dependencies (dep, chain) ->
      fprintf ppf "@[<v>@[Found cyclic dependency: %s@]@,%a@]" dep
        format_dependency_chain chain
  ...
  Location.register_error_of_exn (function
    | Error
        ( ( Missing_module (loc, _, _)
          ...
          | Value_not_found_in_module (loc, _, _) ) as err )
