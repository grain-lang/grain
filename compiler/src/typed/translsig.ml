...
let rec collect_type_vars typ =
  ...
  | TTyPoly (ty_arg, ty_args) ->
      ...
      List.iter collect_type_vars ty_args
  | TTyLink ty_arg | TTySubst ty_arg -> collect_type_vars ty_arg
...
let link_type_vars ty =
  ...
  let rec link_types texpr =
    let desc =
      ...
      | TTyRecord l ->
          TTyRecord (List.map (fun (name, arg) -> (name, link_types arg)) l)
      ...
      | TTyPoly (ty, tyl) -> TTyPoly (link_types ty, List.map link_types tyl)
    in
    { texpr with desc }
  in
  link_types ty
...
let translate_signature sg =
  List.map
    (fun item ->
      ...
      | TSigType _ | TSigModule _ | TSigModType _ -> item)
    sg
