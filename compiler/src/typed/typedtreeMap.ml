...
module MakeMap (Map : MapArgument) : sig
  ...
  let rec map_typed_program tp =
    ...
    Map.leave_typed_program { tp with statements }

  and map_core_type ct =
    let ct = Map.enter_core_type ct in
    let ctyp_desc =
      match ct.ctyp_desc with
      | TTyAny | TTyVar _ -> ct.ctyp_desc
      | TTyArrow (args, ret) ->
      ...
      | TTyRecord args ->
          TTyRecord
            (List.map (fun (name, arg) -> (name, map_core_type arg)) args)
      | TTyPoly (args, typ) ->
  ...
  and map_toplevel_stmt stmt =
    ...
    let ttop_desc =
      match stmt.ttop_desc with
      | TTopData decls -> TTopData (List.map map_data_declaration decls)
      | TTopForeign _ | TTopImport _ | TTopExport _ -> stmt.ttop_desc
      | TTopLet (exportflag, recflag, binds) ->
  ...
  and map_pat_extra (cstr, loc) =
    let cstr =
      match cstr with TPatConstraint ct -> TPatConstraint (map_core_type ct)
    in
    (cstr, loc)
  ...
  and map_pattern pat =
    ...
    let pat_desc =
      ...
      | TPatAny | TPatVar _ | TPatConstant _ -> pat.pat_desc
      ...
      | TPatRecord (fields, c) ->
          TPatRecord
            (List.map (fun (id, ld, pat) -> (id, ld, map_pattern pat)) fields, c)
  ...
  and map_exp_extra (cstr, loc) =
    let cstr =
      match cstr with TExpConstraint ct -> TExpConstraint (map_core_type ct)
    in
    (cstr, loc)
  ...
  and map_expression exp =
    ...
    let exp_desc =
      ...
      | TExpNull | TExpIdent _ | TExpConstant _ -> exp.exp_desc
      ...
      | TExpAssign (be, e) -> TExpAssign (map_expression be, map_expression e)
      ...
      | TExpArraySet (a1, a2, a3) ->
          TExpArraySet (map_expression a1, map_expression a2, map_expression a3)
      | TExpRecord args ->
          TExpRecord
            (Array.map
               (function
                 | desc, Overridden (name, expr) ->
                     (desc, Overridden (name, map_expression expr))
                 | desc, def -> (desc, def))
               args)
