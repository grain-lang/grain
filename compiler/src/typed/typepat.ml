...
let unify_pat_types loc env ty ty' =
  try unify env ty ty'
  with Unify trace -> raise (Error (loc, env, PatternTypeClash trace))
...
let unify_exp_types loc env ty expected_ty =
  ...
  with Unify trace -> raise (Error (loc, env, ExprTypeClash (trace, None)))

(* level at which to create the local type declarations *)
let newtype_level = ref None

let get_newtype_level () =
  match !newtype_level with Some y -> y | None -> assert false

let unify_pat_types_gadt loc env ty ty' =
  let newtype_level =
    match !newtype_level with None -> assert false | Some x -> x
  in
...
let finalize_variant pat = match pat.pat_desc with _ -> ()
...
let pattern_variables =
  ref
    ( []
      : (Ident.t * type_expr * string loc * Location.t * bool)
        (* as-variable *)
        list )
...
let enter_variable ?(is_module = false) ?(is_as_variable = false) loc name ty =
  ...
  if is_module then (
    ...
    if not !allow_modules then raise (Error (loc, Env.empty, ModulesNotAllowed));
...
let enter_orpat_variables loc env p1_vs p2_vs =
  ...
  let rec unify_vars p1_vs p2_vs =
    ...
    | (x, _, _, _, _) :: _, (y, _, _, _, _) :: _ ->
        let err =
          if Ident.name x < Ident.name y then OrpatVars (x, vars p2_vs)
          else OrpatVars (y, vars p1_vs)
        in
...
let rec build_as_type env p =
  ...
  | TPatRecord (lpl, _) ->
      ...
      let do_label lbl =
        ...
        let refinable =
          ...
          && match (repr lbl.lbl_arg).desc with TTyPoly _ -> false | _ -> true
        in
        if refinable then
  ...
  | TPatOr (p1, p2) ->
      ...
      ty1
  | TPatAny | TPatVar _ | TPatConstant _ -> p.pat_type
              ...
              List.map
                (fun ((lid, a) as lid_a) ->
                  ...
                  | Identifier.IdentName s ->
                      ( { lid with txt = Identifier.IdentExternal (modname, s) },
                        a )
                  | _ -> lid_a)
                lid_a_list
        in
...
and type_pat_aux ~constrs ~labels ~no_existentials ~mode ~explode ~env sp
  ...
  | PPatAny ->
      ...
      if explode > 0 then
        ...
        if sp.ppat_desc = Parsetree.PPatAny then k' TPatAny
  ...
  | PPatRecord (lid_pat_list, closed) -> (
      ...
      let type_label_pat (label_lid, label, sarg) k =
        ...
        type_pat sarg ty_arg (fun arg -> k (label_lid, label, arg))
      in
  ...
  | PPatOr (sp1, sp2) -> (
      ...
      match
        ...
        | Some p, None | None, Some p -> p (* No variables *)
        | Some p1, Some p2 ->
            let alpha_env =
              ...
              | TPatVar (id, s) ->
                  {
                    p with
                    pat_type = ty;
                    pat_desc = TPatAlias ({ p with pat_desc = TPatAny }, id, s);
                    pat_extra = [ extra ];
                  }
  ...
  try
    ...
    let typed_p =
      Ctype.with_passive_variants
        (type_pat ~allow_existentials:true ~lev ~constrs ~labels:None ?mode
           ?explode env p)
        expected_ty
    in
    set_state state env;
...
let check_partial ?(lev = get_current_level ()) env expected_ty loc cases =
  ...
  Parmatch.check_partial (partial_pred ~lev ~explode env expected_ty) loc cases

let check_unused ?(lev = get_current_level ()) env expected_ty cases =
  Parmatch.check_unused
    (fun refute constrs spat ->
      match
        partial_pred ~lev ~mode:Split_or ~explode:5 env expected_ty constrs spat
      with
...
let report_type_expected_explanation expl ppf =
  ...
  | Sequence_left_hand_side -> fprintf ppf "the left-hand side of a sequence"
...
let report_error env ppf = function
  | ConstructorArityMismatch (lid, expected, provided) ->
      fprintf ppf
        "@[The constructor %a@ expects %i argument(s),@ but is applied here to \
         %i argument(s)@]"
        identifier lid expected provided
  | PatternTypeClash trace ->
      report_unification_error ppf env trace
        (function ppf -> fprintf ppf "This pattern matches values of type")
        (function
          | ppf ->
              fprintf ppf
  ...
  | ExprTypeClash (trace, explanation) ->
      report_unification_error ppf env trace
        ...
        (function ppf -> fprintf ppf "This expression has type")
        (function ppf -> fprintf ppf "but a expression was expected of type")
  ...
  | ModulesNotAllowed -> fprintf ppf "Modules are not allowed in this pattern."
  ...
  | OrPatternTypeClash (id, trace) ->
      report_unification_error ppf env trace
        (function
          | ppf ->
              fprintf ppf
                "The variable %s on the left-hand side of this or-pattern has \
                 type"
                (Ident.name id))
  ...
  | UnrefutedPattern pat ->
      fprintf ppf "@[%s@ %s@ %a@]" "This match case could not be refuted."
        "Here is an example of a value that would reach it:" Printpat.top_pretty
        pat
