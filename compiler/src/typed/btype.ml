...
let repr t =
  match t.desc with TTyLink t' as d -> repr_link false t d t' | _ -> t
...
let iter_type_expr_kind f = function
  | TDataAbstract -> ()
  | TDataVariant cstrs ->
      List.iter (fun cd -> iter_type_expr_cstr_args f cd.cd_args) cstrs
...
let type_iterators =
  ...
  and it_do_type_expr it ty =
    ...
    match ty.desc with TTyConstr (p, _, _) -> it.it_path p | _ -> ()
  and it_path _p = () in
  {
    it_signature;
    it_signature_item;
...
let rec norm_univar ty =
  ...
  | TTyUniVar _ | TTySubst _ -> ty
  | TTyTuple (ty :: _) -> norm_univar ty
  | TTyRecord _ | TTyVar _ | TTyArrow _ | TTyTuple _ | TTyConstr _ | TTyPoly _
    ->
      assert false
