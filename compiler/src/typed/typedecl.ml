...
let enter_type rec_flag env sdecl id =
  ...
  if not needed then env
    ...
    let decl =
      {
        type_params = List.map (fun _ -> Btype.newgenvar ()) sdecl.pdata_params;
...
let transl_constructor_arguments env closed = function
  | PConstrTuple l ->
      ...
      (Types.TConstrTuple (List.map (fun t -> t.ctyp_type) l), TConstrTuple l)
  ...
  let tkind, kind =
    ...
    | PDataVariant scstrs ->
        ...
        if
          ...
          > Config.max_tag + 1
        then raise (Error (sdecl.pdata_loc, Too_many_constructors));
        let make_cstr scstr =
    ...
    let rec check_regular cpath args prev_exp ty =
      ...
      if not (List.memq ty !visited) then (
        ...
        | TTyConstr (path', args', _) ->
            ...
            else if to_check path' && not (List.mem path' prev_exp) then
              try
                (* Attempt expansion *)
...
let name_recursion sdecl id decl = match decl with _ -> decl
...
let transl_data_decl env rec_flag sdecl_list =
  ...
  *)
  ...
  in
  let decls = List.map (fun tdecl -> (tdecl.data_id, tdecl.data_type)) tdecls in
  current_slot := None;
  ...
  let id_loc_list =
    List.map2 (fun id sdecl -> (id, sdecl.pdata_loc)) id_list sdecl_list
  in
  List.iter
    (fun (id, decl) ->
  ...
  let final_decls =
    List.map2
      (fun tdecl (_id2, decl) -> { tdecl with data_type = decl })
      tdecls final_decls
  in
  (* Done *)
...
let get_native_repr_attribute attrs ~global_repr =
  ...
  match (None, None, global_repr) with
...
let make_native_repr env core_type ty ~global_repr =
  ...
  | Native_repr_attr_present kind ->
      List.fold_left
        (fun _ ty ->
          match native_repr_of_type env kind ty with
          | None ->
              raise
                (Error (Location.dummy_loc, Cannot_unbox_or_untag_type kind))
          | Some repr -> repr)
        Same_as_ocaml_repr ty
    ...
    | [ prim ] ->
        {
          ...
          Types.val_loc = loc;
          val_fullpath =
            Path.PIdent (Ident.create "<bogus>")
            (*val_attributes = valdecl.pval_attributes*);
        }
    | _ ->
        {
...
let report_error ppf = function
  ...
  | Missing_native_external ->
      fprintf ppf
        "@[<hv>An external function with more than 5 arguments requires a \
         second stub function@ for native-code compilation@]"
  ...
  | Extension_mismatch (path, errs) ->
      fprintf ppf "@[<v>@[<hov>%s@ %s@;<1 2>%s@]%a@]" "This extension"
        ...
        (Includecore.report_type_mismatch "the type" "this extension"
           "definition")
        errs
  | Rebind_wrong_type (lid, env, trace) ->
      Printtyp.report_unification_error ppf env trace
        (function
          | ppf ->
              fprintf ppf "The constructor %a@ has type" Printtyp.identifier lid)
        (function ppf -> fprintf ppf "but was expected to be of type")
  | Rebind_mismatch (lid, p, p') ->
      fprintf ppf "@[%s@ %a@ %s@ %s@ %s@ %s@ %s@]" "The constructor"
        Printtyp.identifier lid "extends type" (Path.name p)
        "whose declaration does not match" "the declaration of type"
        (Path.name p')
  | Rebind_private lid ->
      fprintf ppf "@[%s@ %a@ %s@]" "The constructor" Printtyp.identifier lid
        "is private"
  | Bad_variance (n, v1, v2) ->
      ...
      if n = -1 then
        ...
        fprintf ppf "@[%s@ %s@ The %d%s type parameter"
          ...
          "variances are not satisfied." n (suffix n);
      if n <> -2 then
  ...
  | Cannot_unbox_or_untag_type Unboxed ->
      fprintf ppf
        "Don't know how to unbox this type. Only float, int32, int64 and \
         nativeint can be unboxed"
  | Cannot_unbox_or_untag_type Untagged ->
      fprintf ppf "Don't know how to untag this type. Only int can be untagged"
  | Deep_unbox_or_untag_attribute kind ->
      fprintf ppf
        "The attribute '%s' should be attached to a direct argument or result \
  ...
  | Wrong_unboxed_type_float ->
      fprintf ppf
        "@[This type cannot be unboxed because@ it might contain both float \
         and non-float values.@ You should annotate it with \
         [%@%@ocaml.boxed].@]"
  | Boxed_and_unboxed ->
