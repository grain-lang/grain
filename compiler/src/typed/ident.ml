...
type saved_state = { currentstamp : int; reinit_level : int }
...
let save_state () =
  { currentstamp = !currentstamp; reinit_level = !reinit_level }
...
let find_stamp_opt s o = try Some (find_stamp s o) with Not_found -> None
...
let find_same_opt id tbl = try Some (find_same id tbl) with Not_found -> None

let rec find_name name = function
  | Empty -> raise Not_found
  | Node (l, k, r, _) ->
      let c = compare name k.ident.name in
      if c = 0 then (k.ident, k.data)
      else find_name name (if c < 0 then l else r)

let find_name_opt name tbl =
  try Some (find_name name tbl) with Not_found -> None
