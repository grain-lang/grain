          ...
          raise
            (Error
               ( lid.loc,
                 env,
                 WrongName
                   ("", mk_expected (newvar ()), type_kind, tpath, s, names) ))
        )
    | _ -> raise Not_found
              ...
              if paths <> [] then
                warn lid.loc
                  (Warnings.AmbiguousName
                     ([ Identifier.last lid.txt ], paths, false));
              lbl )
              ...
              ( if in_env lbl then
                ...
                warn lid.loc
                  (Warnings.NameOutOfScope
                     (s, [ Identifier.last lid.txt ], false)) );
...
let wrap_disambiguate kind ty f x =
  try f x
  with Error (loc, env, WrongName ("", _, tk, tp, name, valid_names)) ->
...
let report_type_expected_explanation expl ppf =
  ...
  | Assign_not_array -> fprintf ppf "the left-hand side of an array item access"
  | Assign_not_array_index -> fprintf ppf "the argument to an array item access"
...
let report_error env ppf = function
  | WrongName (eorp, ty_expected, kind, p, name, valid_names) ->
      ...
      fprintf ppf "@[@[<2>%s type@ %a%t@]@ " eorp type_expr ty
        (report_type_expected_explanation_opt explanation);
      fprintf ppf "The %s %s does not belong to type %a@]" (label_of_kind kind)
        name (*kind*) path p;
  ...
  | NameTypeMismatch (kind, lid, tp, tpl) ->
      ...
      report_ambiguous_type_error ppf env tp tpl
        (function
          | ppf ->
              fprintf ppf "The %s %a@ belongs to the %s type" name identifier
                lid kind)
        (function
          | ppf ->
        ...
        (function
          | ppf ->
              fprintf ppf "but a %s was expected belonging to the %s type" name
                kind)
