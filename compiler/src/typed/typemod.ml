...
let type_open_ ?used_slot ?toplevel env mod_ =
  ...
  in
  let path =
    Typetexp.lookup_module ~load:true env mod_.pimp_loc mod_name filepath
  in
...
let map_rec_type ~rec_flag fn decls rem =
  ...
  | d1 :: dl ->
      let first =
        match rec_flag with Recursive -> TRecFirst | Nonrecursive -> TRecNot
      in
...
let check cl loc set_ref name =
  if StringSet.mem name !set_ref then
...
let type_module ?(toplevel = false) funct_body anchor env sstr (*scope*) =
  ...
  let string_needs_export (str : string Grain_parsing.Parsetree.loc) =
    ...
    flag && (not @@ List.exists (fun { txt } -> txt = str.txt) excepts)
  in

  let ident_needs_export (id : Ident.t) =
    let flag, excepts, _ = !export_all in
    flag
    && (not @@ List.exists (fun except_id -> id.name = except_id.txt) excepts)
  in
  ...
  let process_foreign env e d loc =
    ...
    in
    let foreign =
      { ttop_desc = TTopForeign desc; ttop_loc = loc; ttop_env = env }
    in
  ...
  let process_primitive env e d loc =
    ...
    let prim =
      {
        ttop_desc = TTopLet (e, Nonrecursive, defs);
        ttop_loc = loc;
        ttop_env = newenv;
      }
    in
  ...
  let process_datas env e datas loc =
    ...
    let ty_decl =
      map_rec_type_with_row_types ~rec_flag:Recursive
        (fun rs info ->
          ...
          | Nonexported ->
              TSigType
                ( info.data_id,
                  { info.data_type with type_kind = TDataAbstract },
                  rs ))
        decls []
    in
    let statement =
      { ttop_desc = TTopData decls; ttop_loc = loc; ttop_env = newenv }
    in
  ...
  let process_let env export_flag rec_flag binds loc =
    ...
    let () =
      if rec_flag = Recursive then Typecore.check_recursive_bindings env defs
    in
    let some_exported = ref false in
    let signatures =
      List.fold_right
        (fun id sigs ->
          if ident_needs_export id || export_flag = Exported then (
            some_exported := true;
    ...
    let stmt =
      {
        ttop_desc = TTopLet (export_flag, rec_flag, defs);
        ttop_loc = loc;
        ttop_env = env;
      }
    in
  ...
  let process_export_value env exports loc =
    let bindings =
      List.map
        (fun { pex_name = name; pex_alias = alias; pex_loc = loc } ->
          let exported_name =
            match alias with Some alias -> alias.txt | None -> name.txt
          in
  ...
  let process_export_data env exports loc =
    let process_one rs { pex_name = name; pex_alias = alias; pex_loc = loc } =
      ...
      | Some alias ->
          type_export_aliases :=
            (type_id, PIdent (Ident.create alias.txt)) :: !type_export_aliases
      | None -> () );
    ...
    else List.map (process_one TRecNot) exports
  in

  let process_export env exports loc =
    let values, datas =
      List.fold_right
        (fun export (values, datas) ->
          match export with
    ...
    in
    let data_sigs =
      if List.length datas > 0 then process_export_data env datas loc else []
    in
    let env, sigs, stmts =
      if List.length values > 0 then process_export_value env values loc
      else (env, [], [])
    in
  ...
  let final_env, signatures, statements =
    List.fold_left
      (fun (env, signatures, statements) { ptop_desc; ptop_loc = loc } ->
        match ptop_desc with
        | PTopImport i ->
  ...
  let resolve_type_alias signature =
    let rec get_alias aliases id =
      match aliases with
      | (name, alias) :: rest ->
          if Path.same name id then Some (name, alias) else get_alias rest id
      | [] -> None
    in
      ...
      | TTyArrow (args, result, c) ->
          {
            ...
            desc =
              TTyArrow
                (List.map resolve_type_expr args, resolve_type_expr result, c);
          }
      | TTyTuple elts ->
          { expr with desc = TTyTuple (List.map resolve_type_expr elts) }
      | TTyRecord elts ->
          {
            expr with
            desc =
              TTyRecord
                (List.map
                   (fun (name, elt) -> (name, resolve_type_expr elt))
                   elts);
          }
      | TTyPoly (one, all) ->
          {
            expr with
            desc =
              TTyPoly (resolve_type_expr one, List.map resolve_type_expr all);
          }
      | TTyConstr (id, args, a) ->
          let name =
            snd @@ Option.default (id, id) @@ get_alias !type_export_aliases id
          in
          {
            expr with
    ...
    match signature with
    | TSigType (id, decl, rs) -> (
        match get_alias !type_export_aliases (PIdent id) with
              ...
              TValConstructor
                {
                  ...
                  cstr_existentials =
                    List.map resolve_type_expr cstr_existentials;
                  cstr_args = List.map resolve_type_expr cstr_args;
                }
          | _ -> val_kind
        in
        TSigValue
          (id, { vd with val_kind; val_type = resolve_type_expr val_type })
    | _ -> signature
  in
...
let implicit_modules : (string * string) list ref =
  ref [ ("pervasives", "pervasives") ]
...
let initial_env () =
  ...
  List.fold_left
    (fun env m ->
      ...
      if unit_name <> modname then open_implicit_module m env else env)
...
let type_implementation prog =
  ...
  let filename = sourcefile in
  (* TODO: I think this is okay *)
  let coercion =
    Includemod.compunit initenv ~mark:Includemod.Mark_positive sourcefile sg
      "(inferred signature)" simple_sg
  in
...
let report_error ppf = function
  | Cannot_apply mty ->
      fprintf ppf "@[This module is not a functor; it has type@ %a@]" modtype
        mty
  | Not_included errs ->
      fprintf ppf "@[<v>Signature mismatch:@ %a@]" Includemod.report_error errs
  | Cannot_eliminate_dependency mty ->
      fprintf ppf
        "@[This functor has type@ %a@ The parameter cannot be eliminated in \
         the result type.@  Please bind the argument to a module identifier.@]"
        modtype mty
  | Signature_expected -> fprintf ppf "This module type is not a signature"
  | Structure_expected mty ->
      fprintf ppf "@[This module is not a structure; it has type@ %a" modtype
        mty
  ...
  | Repeated_name (kind, name) ->
      fprintf ppf
        "@[Multiple definition of the %s name %s.@ Names must be unique in a \
         given structure or signature.@]"
        kind name
  | Non_generalizable typ ->
      fprintf ppf
        "@[The type of this expression,@ %a,@ contains type variables that \
         cannot be generalized@]"
        type_scheme typ
  | Non_generalizable_module mty ->
      fprintf ppf
        "@[The type of this module,@ %a,@ contains type variables that cannot \
         be generalized@]"
        modtype mty
  | Implementation_is_required intf_name ->
      fprintf ppf
        "@[The interface %a@ declares values, not just types.@ An \
  ...
  | Incomplete_packed_module ty ->
      fprintf ppf "The type of this packed module contains variables:@ %a"
        type_expr ty
  | Scoping_pack (lid, ty) ->
      fprintf ppf "The type %a in this module cannot be exported.@ " identifier
        lid;
