...
let extra_pat =
  make_pat (TPatVar (Ident.create "+", mknoloc "+")) Ctype.none Env.empty
  ...
  let coherent_heads hp1 hp2 =
    ...
    | TPatConstant c1, TPatConstant c2 -> (
        ...
        | ( ( Const_int _ | Const_int32 _ | Const_int64 _ | Const_float _
            | Const_bool _ | Const_void | Const_string _ ),
            _ ) ->
            false )
    | TPatTuple l1, TPatTuple l2 -> List.length l1 = List.length l2
    | TPatAny, _ | _, TPatAny -> true
    | _, _ -> false
  in
  match
    List.find
...
let const_compare x y =
  ...
  | ( ( Const_int _ | Const_string _ | Const_float _ | Const_bool _ | Const_void
...
struct
  let rec compat p q =
    ...
    | (TPatAny | TPatVar _), _ | _, (TPatAny | TPatVar _) -> true
...
let get_constructor_type_path ty tenv =
  ...
  match ty.desc with TTyConstr (path, _, _) -> path | _ -> assert false
...
let rec normalize_pat q =
  ...
  | TPatRecord (fields, c) ->
      make_pat
        (TPatRecord (List.map (fun (id, ld, pat) -> (id, ld, omega)) fields, c))
...
let discr_pat q pss =
  let rec refine_pat acc = function
    ...
    | (head, _) :: rows -> (
        ...
        | TPatTuple _ | TPatRecord _ -> normalize_pat head
        | TPatConstant _ | TPatConstruct _ -> acc )
  in
  ...
  match q.pat_desc with TPatAny -> refine_pat q pss | _ -> q
...
let build_specialized_submatrices ~extend_row q rows =
  ...
  let rec insert_constr p r = function
    ...
    | ((q0, rs) as bd) :: env ->
        if simple_match q0 p then extend_group q0 p r rs :: env
...
let set_last a =
  let rec loop = function
    ...
    | x :: l -> x :: loop l
  in
  function _, [] -> (a, []) | first, row -> (first, loop row)
...
let full_match closing env =
  ...
  | ({ pat_desc = TPatTuple _ }, _) :: _ | ({ pat_desc = TPatRecord _ }, _) :: _
    ->
      true
      ...
      | (p, _) :: _ -> (
          ...
          | TPatConstruct
            ...
            ->
              ...
              Path.same path ext
          | TPatConstant _ | TPatTuple _ | TPatRecord _ -> false
          | TPatAny | TPatVar _ | TPatAlias _ | TPatOr _ -> assert false ) )
...
let complete_tags nconsts nconstrs tags =
  ...
  for i = 0 to nconsts - 1 do
    if not seen_const.(i) then ConstructorTagHashtbl.add r (CstrConstant i) ()
  done;
  for i = 0 to nconstrs - 1 do
    if not seen_constr.(i) then ConstructorTagHashtbl.add r (CstrBlock i) ()
  done;
  r
...
let pats_of_type ?(always = false) env ty =
  ...
  | TTyTuple tl -> [ make_pat (TPatTuple (omegas (List.length tl))) ty env ]
  | _ -> [ omega ]
...
let complete_constrs p all_tags =
  let c =
    match p.pat_desc with TPatConstruct (_, c, _) -> c | _ -> assert false
  in
...
let build_other ext env =
  ...
  | (({ pat_desc = TPatConstruct _ } as p), _) :: _ -> (
      ...
      | Some ext ->
          if Path.same ext (get_constructor_type_path p.pat_type p.pat_env) then
            extra_pat
          else build_other_constrs env p
  ...
  | (({ pat_desc = TPatConstant (Const_string _) } as p), _) :: _ ->
      build_other_constant
        (function
          | TPatConstant (Const_string s) -> String.length s | _ -> assert false)
...
let rec has_instance p =
  ...
  | TPatConstruct (_, _, ps) | TPatTuple ps -> has_instances ps
  | TPatRecord (fields, _) ->
...
let rec satisfiable pss qs =
  ...
  | _ -> (
      ...
      | { pat_desc = TPatAlias (q, _, _) } :: qs -> satisfiable pss (q :: qs)
      | { pat_desc = TPatAny | TPatVar _ } :: qs ->
          let pss = simplify_first_col pss in
          if not (all_coherent (first_column pss)) then false
          else
            let { default; constrs } =
              let q0 = discr_pat omega pss in
              build_specialized_submatrices ~extend_row:( @ ) q0 pss
            in
            if not (full_match false constrs) then satisfiable default qs
            else
              List.exists
                (fun (p, pss) ->
      ...
      | q :: qs ->
          let pss = simplify_first_col pss in
          if not (all_coherent (q :: first_column pss)) then false
          else
...
type 'a exhaust_result = No_matching_value | Witnesses of 'a list
...
*)
let rec exhaust (ext : Path.t option) pss n =
  match pss with
  | [] -> Witnesses [ omegas n ]
  | [] :: _ -> No_matching_value
  | pss -> (
      ...
      if not (all_coherent (first_column pss)) then
        ...
        | { default; constrs = [] } -> (
            (* first column of pss is made of variables only *)
            match exhaust ext default (n - 1) with
            | Witnesses r -> Witnesses (List.map (fun row -> q0 :: row) r)
            | r -> r )
        | { default; constrs } -> (
            let try_non_omega (p, pss) =
              match
                exhaust ext pss (List.length (simple_match_args p omega) + n - 1)
              with
              | Witnesses r ->
...
let exhaust ext pss n =
  ...
  | Witnesses lst ->
      let singletons =
        List.map (function [ x ] -> x | _ -> assert false) lst
      in
...
let rec pressure_variants tdefs = function
  ...
  | pss -> (
      let pss = simplify_first_col pss in
      if not (all_coherent (first_column pss)) then true
      else
        let q0 = discr_pat omega pss in
        match build_specialized_submatrices ~extend_row:( @ ) q0 pss with
        ...
        | { default; constrs } ->
            ...
            else if tdefs = None then pressure_variants None default
            else
...
type answer =
  ...
  | Upartial of Typedtree.pattern list
...
let rec unalias p =
  match p.pat_desc with TPatAlias (p, _, _) -> unalias p | _ -> p

let is_var p =
  match (unalias p).pat_desc with TPatAny | TPatVar _ -> true | _ -> false

let is_var_column rs =
  List.for_all
    (fun r -> match r.active with p :: _ -> is_var p | [] -> assert false)
    rs
...
let remove r =
  match r.active with _ :: rem -> { r with active = rem } | [] -> assert false
...
let rec simplify_first_usefulness_col = function
  ...
  | row :: rows -> (
      ...
      | p :: ps ->
          let add_column p ps k = (p, { row with active = ps }) :: k in
...
let rec every_satisfiables pss qs =
  ...
  | [] -> (
      ...
      | [] ->
          (* no or-patterns *)
          if satisfiable (make_matrix pss) (make_vector qs) then Used
          else Unused
      | _ ->
  ...
  | q :: rem -> (
      ...
      | _ ->
          ...
          (* The handling of incoherent matrices is kept in line with
             [satisfiable] *)
          if not (all_coherent (uq :: first_column pss)) then Unused
          else
            let q0 = discr_pat q pss in
            every_satisfiables
              (build_specialized_submatrix q0 pss ~extend_row:(fun ps r ->
                   { r with active = ps @ r.active }))
...
and every_both pss qs q1 q2 =
  ...
  | Used -> ( match r2 with Unused -> Upartial [ q2 ] | _ -> r2 )
  | Upartial u1 -> (
      match r2 with
...
and orlub p1 p2 q =
  try
    let r1 = lub p1 q in
    try { q with pat_desc = TPatOr (r1, lub p2 q) } with Empty -> r1
  with Empty -> lub p2 q

and lubs ps qs =
  match (ps, qs) with p :: ps, q :: qs -> lub p q :: lubs ps qs | _, _ -> []
...
module Conv = struct
  ...
  let conv typed =
    ...
    let rec loop pat =
      ...
      | TPatAny | TPatVar _ -> mkpat PPatAny
      ...
      | TPatRecord (fields, c) ->
          mkpat
            (PPatRecord (List.map (fun (id, _, pat) -> (id, loop pat)) fields, c))
...
let do_check_partial ~pred loc (casel : match_branch list) pss =
  match pss with
  | [] ->
      (*
          This can occur
  ...
  | ps :: _ -> (
      ...
      | Witnesses [ u ] -> (
          ...
          | None -> Total
          | Some v ->
              ( if Warnings.is_active (Warnings.PartialMatch "") then
                let errmsg =
                  try
...
let rec collect_paths_from_pat r p =
  match p.pat_desc with
  | TPatConstruct
      (_, { cstr_tag = CstrConstant _ | CstrBlock _ | CstrUnboxed }, ps) ->
      let path = get_constructor_type_path p.pat_type p.pat_env in
  ...
  | TPatOr (p1, p2) -> collect_paths_from_pat (collect_paths_from_pat r p1) p2
...
let do_check_fragile loc casel pss =
  let exts =
    List.fold_left (fun r c -> collect_paths_from_pat r c.mb_pat) [] casel
  in
  match exts with
  | [] -> ()
  | _ -> (
...
let check_unused pred casel =
  if
    Warnings.is_active Warnings.UnusedMatch
          ...
          ( try
              ...
              if r = Unused && refute then ()
                ...
                let r =
                  ...
                  if skip then r
                    ...
                    if sfs = [] then Unused
                      ...
                      let sfs =
                        List.map (function [ u ] -> u | _ -> assert false) sfs
                      in
                      ...
                      | None when not refute ->
                          Location.prerr_warning q.pat_loc
                ...
                | Upartial ps ->
                    List.iter
                      (fun p ->
                        Location.prerr_warning p.pat_loc Warnings.UnusedPat)
                      ps
                | Used -> ()
...
let inactive ~partial pat =
  ...
  | Total ->
      let rec loop pat =
        ...
        | TPatConstant c -> (
            match c with
            | Const_string _ | Const_void | Const_int _ | Const_bool _
...
let check_partial pred loc casel =
  ...
  if total = Total && Warnings.is_active (Warnings.FragileMatch "") then
...
let rec simplify_first_amb_col = function
  ...
  | Positive { row = p :: ps; varsets } :: rem ->
      ...
      simplify_head_amb_pat Ident.Set.empty varsets ~add_column p ps
        (simplify_first_amb_col rem)

(* Compute stable bindings *)

type stable_vars = All | Vars of Ident.Set.t
...
let rec matrix_stable_vars m =
  ...
  | (Positive { row = []; _ } | Negative []) :: _ as empty_rows -> (
      ...
      let get_varsets = function
        ...
        | Positive p ->
            ...
            p.varsets
      in
      match List.map get_varsets empty_rows with
  ...
  | m ->
      let is_negative = function Negative _ -> true | Positive _ -> false in
      if List.for_all is_negative m then
        ...
        if not (all_coherent (first_column m)) then All
        else
          ...
          let submatrices =
            let extend_row columns = function
              ...
              | Positive r -> Positive { r with row = columns @ r.row }
            in
            ...
            if full_match false constrs then non_default
            else default :: non_default
          in
...
let all_rhs_idents exp =
  ...
  let module Iterator = TypedtreeIter.MakeIterator (struct
    ...
    let leave_expression exp =
      if is_unpack exp then match exp.exp_desc with _ -> assert false
  end) in
