...
let loc s x = if s.for_saving then Location.dummy_loc else x
...
let rec typexp s ty =
  ...
  | _ ->
      ...
      (* Make a stub *)
      let ty' =
        if s.for_saving then newpersty (TTyVar None) else newgenvar ()
      in
      ty.desc <- TTySubst ty';
      ty'.desc <-
        ( match desc with
...
let record_field s f =
  { rf_name = f.rf_name; rf_type = typexp s f.rf_type; rf_loc = loc s f.rf_loc }
  ...
  let decl =
    {
      ...
      type_kind =
        ...
        | TDataRecord fields -> TDataRecord (List.map (record_field s) fields)
        | TDataVariant cstrs ->
...
let rec modtype s = function
  | (TModIdent p as mty) | (TModAlias p as mty) -> (
      match p with
      | PIdent id -> ( try Tbl.find id s.modtypes with Not_found -> mty )
      | PExternal (p, n, pos) -> TModIdent (PExternal (module_path s p, n, pos))
      )
...
and signature_component s comp newid =
  ...
  | TSigValue (_id, d) ->
      TSigValue
        (newid, { (value_description s d) with val_fullpath = Path.PIdent _id })
  | TSigType (_id, d, rs) -> TSigType (newid, type_declaration s d, rs)
  | TSigModule (_id, d, rs) -> TSigModule (newid, module_declaration s d, rs)
...
and modtype_declaration s decl =
  { mtd_type = may_map (modtype s) decl.mtd_type; mtd_loc = loc s decl.mtd_loc }
