...
type t = PIdent of Ident.t | PExternal of t * string * int
...
let rec compare p1 p2 =
  ...
  | PExternal (mod1, s1, _), PExternal (mod2, s2, _) ->
      ...
      if s_comp <> 0 then s_comp else compare mod1 mod2
...
let rec head = function PIdent id -> id | PExternal (m, _, _) -> head m

let heads p =
  let rec heads p acc =
    match p with PIdent id -> id :: acc | PExternal (m, _, _) -> heads m acc
  in
  heads p []

let rec last = function PIdent id -> Ident.name id | PExternal (_, s, _) -> s
