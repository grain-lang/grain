...
(** [commutable] is a flag appended to every arrow type.
    ...
    the actual type.  
*)
type commutable = TComOk | TComUnknown | TComLink of commutable ref
...
and type_desc =
  | TTyVar of string option  (** A type variable (None == "_")*)
  | TTyArrow of type_expr list * type_expr * commutable  (** A function type. *)
  | TTyTuple of type_expr list  (** A tuple type. *)
  | TTyRecord of (string * type_expr) list  (** A record type. *)
  ...
  | TTyLink of type_expr  (** Used internally by the unification engine. *)
  | TTySubst of type_expr  (** Used internally by the unification engine. *)
...
and abbrev_memo =
  ...
  | TMemLink of abbrev_memo ref
      (** Abbreviations can be found after this indirection *)

type constructor_tag = CstrConstant of int | CstrBlock of int | CstrUnboxed
[@@deriving sexp, yojson]
...
and value_unbound_reason = ValUnboundGhostRecursive
...
let rec value_kind_to_yojson = function
  ...
  | TValConstructor d ->
      `List [ `String "TValConstructor"; constructor_description_to_yojson d ]
...
and value_kind_of_yojson =
  ...
  | `List [ `String "TValUnbound"; r ] ->
      res_map (fun r -> TValUnbound r) (value_unbound_reason_of_yojson r)
  | `List [ `String "TValConstructor"; d ] ->
      res_map (fun d -> TValConstructor d) (constructor_description_of_yojson d)
  | other ->
      Result.Error
...
and constructor_arguments = TConstrTuple of type_expr list | TConstrSingleton
...
type rec_status = TRecNot | TRecFirst | TRecNext [@@deriving sexp, yojson]
...
let equal_tag t1 t2 =
  match (t1, t2) with
  | CstrBlock i1, CstrBlock i2 | CstrConstant i1, CstrConstant i2 -> i1 = i2
  | CstrUnboxed, CstrUnboxed -> true
  | _ -> false

let may_equal_constr c1 c2 =
  match (c1.cstr_tag, c2.cstr_tag) with tag1, tag2 -> equal_tag tag1 tag2
