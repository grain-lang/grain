...
module PathTbl = {
  ...
  let add: (t('a), (string, string), 'a) => unit =
    (tbl, (dir, unit_name), v) => {
      let dir = Grain_utils.Filepath.realpath_quick(dir);
      Hashtbl.add(tbl, Grain_utils.Filepath.smart_cat(dir, unit_name), v);
    };
  ...
  let find_opt:
    ...
    (~disable_relpath=false, tbl, base_path, path, unit_name) =>
      if (!disable_relpath && is_relpath(unit_name)) {
        Hashtbl.find_opt(
          tbl,
          Grain_utils.Filepath.canonicalize_relpath(base_path, unit_name),
        );
      } else {
        List.fold_left(
          (acc, elt) => {
...
let log_resolution = (unit_name, dir, basename) => {
  let resolution =
    Grain_utils.Filepath.derelativize @@ Filename.concat(dir, basename);
...
let located_to_out_file_name = (~base=?, located) => {
  ...
  Grain_utils.Filepath.derelativize(~base?, ret);
};
    ...
    let compile_module = (~loc=?, dn) => {
      let srcpath =
        ...
        | Some(GrainModule(srcpath, _)) =>
          Grain_utils.Filepath.derelativize(srcpath)
        };
