    ...
    | PTopExpose(items) =>
      let items =
        ...
        | PExposeItems(items) =>
          ...
          | _ =>
            let get_loc = (item: Parsetree.expose_item) => {
              switch (item) {
              | PExposeValue({loc})
              | PExposeModule({loc})
              | PExposeType({loc}) => loc
            ...
            let print_item = (~comments, item: Parsetree.expose_item) => {
              let item_name = (name, alias) => {
                Location.(
                  switch (alias) {
                  | None => print_ident(name.txt)
              ...
              switch (item) {
              | PExposeValue({name, alias})
              | PExposeModule({name, alias}) => item_name(name, alias)
              | PExposeType({name, alias}) =>
                Doc.concat([Doc.text("type "), item_name(name, alias)])
              };
            };
      ...
      Doc.concat([Doc.text("expose"), Doc.space, items]);
    | PTopModule(export_flag, md) =>
