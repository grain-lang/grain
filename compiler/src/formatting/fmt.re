...
let print_grouped_access_expression = (fmt, expr) =>
  ...
  | PExpApp(func, _) when is_infix_op(func) =>
    parens(fmt.print_expression(fmt, expr))
  | PExpApp(_) => fmt.print_expression(fmt, expr)
  | _ => parens(fmt.print_expression(fmt, expr))
  };
...
let print_match_branch = (fmt, {pmb_pat, pmb_body, pmb_guard}) => {
  let (guard, guard_loc) =
    ...
    | Some(guard) => (
        ...
        ++ string("when ")
        ++ fmt.print_expression(fmt, guard),
        guard.pexp_loc,
      )
    };
  let space_type =
  ...
  group(fmt.print_pattern(fmt, pmb_pat))
  ++ guard
  ++ string(" =>")
  ++ indent(
       2,
       ...
       ++ group(fmt.print_expression(fmt, pmb_body)),
     );
};
  ...
  if (force_blocks) {
    let true_branch_doc =
      ...
      | _ =>
        block_braces(
          ~lead=hardline,
          ~trail=hardline,
          fmt.print_expression(fmt, true_branch),
        )
      };
      ...
      | Some(false_branch) =>
        Some(
          block_braces(
            ~lead=hardline,
            ~trail=hardline,
            fmt.print_expression(fmt, false_branch),
          ),
        )
      | None => None
      };
    ...
    | (_, Some(false_branch)) =>
      ...
      group(
        ...
        ++ indent(
             2,
             breakable_space ++ fmt.print_expression(fmt, false_branch),
           ),
      );
    };
  };
  ...
  ++ (
    ...
    | PExpApp(fn, [arg]) when is_prefix_op(fn) =>
      ...
      ++ (
        ...
        | ParenGrouping => parens(fmt.print_application_argument(fmt, arg))
        | FormatterGrouping =>
               ...
               ~f=
                 (~final, a) =>
                   if (final) {
                     ...
                     group(fmt.print_application_argument(fmt, a) ++ comma);
                   },
               exprs,
             ),
           ),
    ...
    | PExpLambda(
      ...
      ++ group(
           ...
           | _ =>
             indent(2, breakable_space ++ fmt.print_expression(fmt, body))
           },
         )
    ...
    | PExpArrayGet(arr, elem) =>
      ...
      ++ list_brackets(fmt.print_expression(fmt, ~infix_wrap=Fun.id, elem))
    ...
    | PExpFor(init, cond, inc, body) =>
      ...
      parens(
        ...
        ~trail=space ++ fmt.print_expression(fmt, body),
      );
    | PExpMatch(value, {txt: branches, loc: branches_loc}) =>
      ...
      ++ block_braces(
           ...
           concat_map(
             ...
             ~f=
               (~final, b) =>
                 group(fmt.print_match_branch(fmt, b) ++ comma),
             branches,
           ),
         )
    ...
    parens(
      concat_map(
        ...
        ~f=
          (~final, a) =>
            if (final) {
              group(fmt.print_parsed_type_argument(fmt, a)) ++ trailing_comma;
            } else {
         ...
         ++ braces(
              concat_map(
                ...
                ~f=
                  (~final, p) =>
                    if (final) {
                      group(fmt.print_provide_item(fmt, p)) ++ trailing_comma;
                    } else {
