...
type input_source = InputString of string | InputFile of string
...
type compilation_action = Continue of compilation_state | Stop
...
let next_state ({ cstate_desc; cstate_filename } as cs) =
  let cstate_desc =
    ...
    | Linearized anfed ->
        ...
        else Optimized anfed
    | Optimized optimized -> Mashed (Transl_anf.transl_anf_program optimized)
    | Mashed mashed ->
        Compiled (Compmod.compile_wasm_module ?name:cstate_filename mashed)
    | Compiled compiled ->
        ( if !Grain_utils.Config.output_enabled then
...
let () =
  Env.compile_module_dependency :=
    fun input outfile -> ignore (compile_file ~outfile ~reset:false input)
