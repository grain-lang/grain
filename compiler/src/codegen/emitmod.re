...
let emit_module = ({asm, signature}, outfile: Filepath.t) => {
  ...
  if (Config.wat^) {
    ...
    let wat_file = Filepath.append(dir, basename ++ ".wat");
  ...
  switch (map) {
  | Some(map) =>
    let map_file = Filepath.append(dir, basename ++ ext ++ ".map");
