...
let binaryFileExtensions = (actual:string, {createMatcher}) => {
  let createBinaryFilesMatcher = createMatcher(
      ( {matcherHint, formatReceived, formatExpected, indent, prepareDiff}, actualThunk, expectedThunk) => {
      ...
      let expected_string = Bytes.to_string(Buffer.to_bytes(expected_buffer));
  ...
  {
    toBinaryMatch: (expected) => createBinaryFilesMatcher(() => "", () => expected),
  };
};