...
let wrap_todo todo f x =
  match todo with Some msg -> OUnit2.todo msg | None -> f x
...
let tlib ?todo ?returns ?code input_file =
  input_file >:: wrap_todo todo @@ test_run_stdlib ?returns ?code input_file

let tgcfile ?todo name heap_size input_file expected =
  name >:: wrap_todo todo @@ test_run_file ~heap_size input_file name expected

let test_final_anf program_str outfile
    (expected : Grain_middle_end.Anftree.anf_expression) test_ctxt =
  let open Grain_middle_end in
  let final_anf =
    Anf_utils.clear_locations @@ compile_string_to_final_anf outfile program_str
  in
  ...
  let result, expected =
    try
      ...
      in
      let expected =
        Sexplib.Sexp.to_string_hum @@ Anftree.sexp_of_anf_expression expected
      in
      (result, expected)
    with e ->
...
let tfinalanf name ?todo (program : string)
...
let tfsound ?todo name filename expected =
  name
  >:: wrap_todo todo @@ test_file_optimizations_sound filename name expected

let test_parse ?todo name input
    (expected : Grain_parsing.Parsetree.parsed_program) test_ctxt =
  (match todo with Some msg -> OUnit2.todo msg | _ -> ());
  let open Grain_parsing in
  let location_stripper =
    {
      Ast_mapper.default_mapper with
      location = (fun _ _ -> Location.dummy_loc);
    }
  in
  ...
  assert_equal expected untagged ~printer:(fun p ->
      Sexplib.Sexp.to_string_hum
...
let function_tests =
  [
    ...
    te "shadow_multi" "let x = 12, x = 14; x"
...
let array_tests =
  [
    ...
    te "array_access_err" "let x = [> 1, 2, 3]; x[3]"
      "array index out of bounds";
    te "array_access_err2" "let x = [> 1, 2, 3]; x[-4]"
      "array index out of bounds";
    te "array_access_err3" "let x = [> 1, 2, 3]; x[99]"
      "array index out of bounds";
    te "array_access_err4" "let x = [> 1, 2, 3]; x[-99]"
      "array index out of bounds";
    ...
    te "array_set_err2" "let x = [> 1, 2, 3]; x[-12] := 4"
      "array index out of bounds";
    te "array_type" "let x = [> true, false, false]; x[1] + 3"
      "has type Bool but";
    te "array_type2" "let x = [> true, false, false]; (x[1] := true) + 3"
      "has type Bool but";
  ]
...
let record_tests =
  [
    ...
    t "record_multiple"
      "export data Rec = {foo: Number, bar: String, baz: Bool}; {foo: 4, bar: \
       'boo', baz: true}"
      "{\n  foo: 4,\n  bar: \"boo\",\n  baz: true\n}";
    t "record_pun" "export data Rec = {foo: Number}; let foo = 4; {foo}"
      "{\n  foo: 4\n}";
    t "record_pun_multiple"
      "export data Rec = {foo: Number, bar: Bool}; let foo = 4; let bar = \
       false; {foo, bar}"
      "{\n  foo: 4,\n  bar: false\n}";
    t "record_pun_mixed"
      "export data Rec = {foo: Number, bar: Bool}; let foo = 4; {foo, bar: \
       false}"
      "{\n  foo: 4,\n  bar: false\n}";
    t "record_pun_mixed_2"
      "export data Rec = {foo: Number, bar: Bool}; let bar = false; {foo: 4, \
       bar}"
      "{\n  foo: 4,\n  bar: false\n}";
    te "record_err_1" "{foo: 4}" "Unbound record label foo";
    te "record_err_2" "data Rec = {foo: Number}; {foo: 4, bar: 4}"
    ...
    t "record_get_multiple"
      "data Rec = {foo: Number, bar: Number}; let x = {foo: 4, bar: 9}; x.foo \
       + x.bar"
      "13";
    t "record_get_multilevel"
      "data Rec1 = {foo: Number, bar: Number}; data Rec2 = {baz: Rec1}; let x \
       = {baz: {foo: 4, bar: 9}}; x.baz.bar"
      "9";
    te "record_get_err"
    ...
    t "record_destruct_1"
      "data Rec = {foo: Number, bar: String, baz: Bool}; let { foo, _ } = \
       {foo: 4, bar: 'boo', baz: true}; foo"
      "4";
    t "record_destruct_2"
      "data Rec = {foo: Number, bar: String, baz: Bool}; let { bar, _ } = \
       {foo: 4, bar: 'boo', baz: true}; bar"
      "\"boo\"";
    t "record_destruct_3"
      "data Rec = {foo: Number, bar: Number, baz: Number}; let { foo, bar, _ } \
       = {foo: 4, bar: 5, baz: 6}; foo + bar"
      "9";
    t "record_destruct_4"
      "data Rec = {foo: Number, bar: Number, baz: Number}; let { foo, bar, baz \
       } = {foo: 4, bar: 5, baz: 6}; foo + bar + baz"
      "15";
    t "record_destruct_deep"
      "data Rec = {foo: Number}; data Rec2 = {bar: Rec}; let { bar: { foo } } \
       = {bar: {foo: 4}}; foo"
      "4";
    te "record_destruct_deep_alias"
      "data Rec = {foo: Number}; data Rec2 = {bar: Rec}; let { bar: { foo } } \
       = {bar: {foo: 4}}; bar"
      "Unbound value bar";
  ]
...
let stdlib_tests =
  [
    ...
    t "stdlib_equal_12"
      "import * from 'lists'; [> 1, 2, 3, 4] == [> 1, 2, 3, 4]" "true";
    ...
    t "stdlib_equal_18" "import * from 'lists'; 'foo 😂' == 'foo 🙄'"
      "false";
    t "stdlib_equal_19"
      "data Rec = {foo: Number, bar: String, baz: Bool}; {foo: 4, bar: 'boo', \
       baz: true} == {foo: 4, bar: 'boo', baz: true}"
      "true";
    t "stdlib_equal_20"
      "data Rec = {foo: Number, bar: String, baz: Bool}; {foo: 4, bar: 'boo', \
       baz: true} == {foo: 4, bar: 'bar', baz: true}"
      "false";
    t "stdlib_equal_21"
      "data Rec = {foo: Number, bar: String, baz: Bool}; {foo: 4, bar: 'boo', \
       baz: true} == {foo: 78, bar: 'boo', baz: true}"
      "false";
    t "stdlib_equal_22"
      "data Rec = {foo: Number, bar: String, baz: Bool}; {foo: 4, bar: 'boo', \
       baz: true} == {foo: 4, bar: 'boo', baz: false}"
      "false";
    ...
    te "stdlib_length_err" "import * from 'lists'; length(true)"
      "This expression has type Bool but";
    te "stdlib_reverse_err" "import * from 'lists'; reverse(1)"
...
let match_tests =
  [
    ...
    t "tuple_match_3" "match ((1, 'boop', false)) { | (a, b, c) => (a, b, c) }"
      "(1, \"boop\", false)";
    t "tuple_match_deep"
      "match ((1, (4, 5), 3)) { | (a, (d, e), c) => a + c + d + e }" "13";
    t "tuple_match_deep2"
      "match ((1, (2, (3, (4, 5, (6, 7)))))) { | (a, (b, (c, (d, e, (f, g))))) \
       => a + b + c + d + e + f + g }"
      "28";
    t "tuple_match_deep3"
      "match ((1, [])) { | (a, []) => a | (a, [b]) => a + b | (a, [b, c]) => a \
       + b + c | (a, [b, c, d]) => a + b + c + d | (_, [_, ..._]) => 999 }"
      "1";
    t "tuple_match_deep4"
      "match ((1, [2])) { | (a, []) => a | (a, [b]) => a + b | (a, [b, c]) => \
       a + b + c | (a, [b, c, d]) => a + b + c + d | (_, [_, ..._]) => 999 }"
      "3";
    t "tuple_match_deep5"
      "match ((1, [4, 5])) { | (a, []) => a | (a, [b]) => a + b | (a, [b, c]) \
       => a + b + c | (a, [b, c, d]) => a + b + c + d | (_, [_, ..._]) => 999 \
       }"
      "10";
    t "tuple_match_deep6"
      "match ((1, [4, 5, 6])) { | (a, []) => a | (a, [b]) => a + b | (a, [b, \
       c]) => a + b + c | (a, [b, c, d]) => a + b + c + d | (_, [_, ..._]) => \
       999 }"
      "16";
    t "tuple_match_deep7"
      "match ((1, [4, 5, 6, 7])) { | (a, []) => a | (a, [b]) => a + b | (a, \
       [b, c]) => a + b + c | (a, [b, c, d]) => a + b + c + d | (_, [_, ..._]) \
       => 999 }"
      "999";
    (* Pattern matching on records *)
    t "record_match_1"
      "data Rec = {foo: Number, bar: String, baz: Bool}; match ({foo: 4, bar: \
       'boo', baz: true}) { | { foo, _ } => foo }"
      "4";
    t "record_match_2"
      "data Rec = {foo: Number, bar: String, baz: Bool}; match ({foo: 4, bar: \
       'boo', baz: true}) { | { bar, _ } => bar }"
      "\"boo\"";
    t "record_match_3"
      "data Rec = {foo: Number, bar: Number, baz: Number}; match ({foo: 4, \
       bar: 5, baz: 6}) { | { foo, bar, _ } => foo + bar }"
      "9";
    t "record_match_4"
      "data Rec = {foo: Number, bar: Number, baz: Number}; match ({foo: 4, \
       bar: 5, baz: 6}) { | { foo, bar, baz } => foo + bar + baz}"
      "15";
    t "record_match_deep"
      "data Rec = {foo: Number}; data Rec2 = {bar: Rec}; match ({bar: {foo: \
       4}}) { | { bar: { foo } } => foo }"
      "4";
    te "record_match_deep_alias"
      "data Rec = {foo: Number}; data Rec2 = {bar: Rec}; match ({bar: {foo: \
       4}}) { | { bar: { foo } } => bar }"
      "Unbound value bar";
    (* Pattern matching on ADTs *)
    t "adt_match_1"
      "match ([]) { | [] => 0 | [b] => b | [b, c] => b + c | [b, c, d] => b + \
       c + d | [_, ..._] => 999 }"
      "0";
    t "adt_match_2"
      "match ([2]) { | [] => 0 | [b] => b | [b, c] => b + c | [b, c, d] => b + \
       c + d | [_, ..._] => 999 }"
      "2";
    t "adt_match_3"
      "match ([4, 5]) { | [] => 0 | [b] => b | [b, c] => b + c | [b, c, d] => \
       b + c + d | [_, ..._] => 999 }"
      "9";
    t "adt_match_4"
      "match ([4, 5, 6]) { | [] => 0 | [b] => b | [b, c] => b + c | [b, c, d] \
       => b + c + d | [_, ..._] => 999 }"
      "15";
    t "adt_match_5"
      "match ([4, 5, 6, 7]) { | [] => 0 | [b] => b | [b, c] => b + c | [b, c, \
       d] => b + c + d | [_, ..._] => 999 }"
      "999";
    t "adt_match_deep"
      "data Rec = {foo: Number}; match ([{foo: 5}]) { | [] => 999 | [{foo}, \
       ..._] => foo }"
      "5";
...
let import_tests =
  [
    (* import * tests *)
    t "import_all" "import * from 'exportStar'; {print(x); print(y(4)); z}"
      "5\n4\n\"foo\"";
    t "import_all_except" "import * except {y} from 'exportStar'; {print(x); z}"
      "5\n\"foo\"";
    t "import_all_except_multiple" "import * except {x, y} from 'exportStar'; z"
      "\"foo\"";
    t "import_all_constructor" "import * from 'tlists'; Cons(2, Empty)"
      "Cons(2, Empty)";
    t "import_all_except_constructor"
      "import * except {Cons} from 'tlists'; Empty" "Empty";
    t "import_all_except_multiple_constructor"
      "import * except {Cons, append} from 'tlists'; sum(Empty)" "0";
    t "import_with_export_multiple" "import * from 'sameExport'; foo()" "6";
    (* import * errors *)
    te "import_all_except_error"
      "import * except {y} from 'exportStar'; {print(x); print(y); z}"
      "Unbound value y";
    te "import_all_except_multiple_error"
      "import * except {x, y} from 'exportStar'; {print(x); z}"
      "Unbound value x";
    te "import_all_except_multiple_error2"
      "import * except {x, y} from 'exportStar'; {print(x); print(y); z}"
      "Unbound value y";
    te "import_all_except_error_constructor"
      "import * except {Cons} from 'tlists'; Cons(2, Empty)"
      "Unbound value Cons";
    te "import_all_except_multiple_error_constructor"
      "import * except {Cons, append} from 'tlists'; append(Empty, Empty)"
      "Unbound value append";
    te "import_all_except_multiple_error2_constructor"
      "import * except {Cons, append} from 'tlists'; let x = Cons(2, Empty); \
       append(x, Empty)"
      "Unbound value Cons";
    (* import {} tests *)
    t "import_some" "import {x} from 'exportStar'; x" "5";
    t "import_some_multiple" "import {x, y} from 'exportStar'; y(x)" "5";
    t "import_some_constructor"
      "import {Cons, Empty} from 'tlists'; Cons(5, Empty)" "Cons(5, Empty)";
    t "import_some_mixed"
      "import {Cons, Empty, sum} from 'tlists'; sum(Cons(5, Empty))" "5";
    t "import_alias" "import {x as y} from 'exportStar'; y" "5";
    t "import_alias_multiple" "import {x as y, y as x} from 'exportStar'; x(y)"
      "5";
    t "import_alias_constructor"
      "import {Empty as None, sum} from 'tlists'; sum(None)" "0";
    t "import_alias_multiple_constructor"
      "import {Cons as Add, Empty as None, sum} from 'tlists'; sum(Add(1, \
       None))"
      "1";
    (* import {} errors *)
    te "import_some_error" "import {a} from 'exportStar'; a"
      "Export 'a' was not found in 'exportStar'";
    te "import_some_error2" "import {x, a} from 'exportStar'; a"
      "Export 'a' was not found in 'exportStar'";
    te "import_some_error3" "import {Foo} from 'exportStar'; a"
      "Export 'Foo' was not found in 'exportStar'";
    te "import_some_error3" "import {x, Foo} from 'exportStar'; a"
      "Export 'Foo' was not found in 'exportStar'";
    (* import module tests *)
    t "import_module" "import Foo from 'exportStar'; Foo.x" "5";
    t "import_module2" "import Foo from 'exportStar'; Foo.y(Foo.x)" "5";
    (* import module errors *)
    te "import_module_error" "import Foo from 'exportStar'; Foo.foo"
      "Unbound value foo in module Foo";
    (* import well-formedness errors *)
    te "import_alias_illegal_renaming"
      "import {Cons as cons, Empty} from 'lists'; cons(3, Empty)"
      "Alias 'cons' should have proper casing";
    te "import_alias_illegal_renaming2"
      "import {sum as Sum, Empty} from 'lists'; sum(Empty)"
      "Alias 'Sum' should have proper casing";
    te "import_module_illegal_name" "import foo from 'lists';"
      "Module 'foo' should have an uppercase name";
    te "import_module_not_external" "import Foo.Foo from 'lists';"
      "Module name 'Foo.Foo' should contain only one module.";
    te "import_value_not_external" "import {foo as Foo.foo} from 'lists';"
      "Alias 'Foo.foo' should be at most one level deep";
    (* import multiple modules tests *)
    t "import_muliple_modules"
      "import * from 'tlists'; import * from 'exportStar'; Cons(x, Empty)"
      "Cons(5, Empty)";
    (* import same module tests *)
    t "import_same_module_unify"
      "import * from 'tlists'; import TList from 'tlists'; Cons(5, TList.Empty)"
      "Cons(5, Empty)";
    t "import_same_module_unify2"
      "import *, TList from 'tlists'; Cons(5, TList.Empty)" "Cons(5, Empty)";
    ...
    te "import_missing_file" "import * from 'foo'; 2"
...
let optimization_tests =
  [
    ...
    t "trs1"
      ...
      \         f2 = ((x, y) => {y});\n\
      \       f1(1, 2)" "1";
    tfinalanf "test_dead_branch_elimination_1" "{ if (true) {4} else {5} }"
      (AExp.comp (Comp.imm (Imm.const (Const_int 4))));
    tfinalanf "test_dead_branch_elimination_2" "{ if (false) {4} else {5} }"
    ...
    tfinalanf "test_const_propagation"
      ...
      (AExp.comp (Comp.imm (Imm.const (Const_int 4))));
    (* Primarily a constant-propagation test, but DAE removes the let bindings as well *)
    tfinalanf "test_const_propagation2"
      "((x) => {\n    let x = 4;\n    let y = x;\n    x})"
    ...
    tfinalanf "test_cse" "((x) => {let a = x + 1; let b = x + 1; a + b})"
      ...
      AExp.comp
        (Comp.lambda [ x ]
           (AExp.let_ Nonrecursive
              [
                (a, Comp.app (Imm.id plus) [ Imm.id x; Imm.const (Const_int 1) ]);
              ]
    ...
    te "test_const_fold_times_zero_sound" "let f = ((x) => {x * 0}); f(false)"
      "Number";
    ...
    te "test_const_fold_times_one_sound" "let f = ((x) => {x * 1}); f(true)"
      "Number";
...
let string_tests =
  ...
  [
    tparse "string_parse_dqs1" "\"foo\""
      { statements = [ str "foo" ]; prog_loc = Location.dummy_loc };
    tparse "string_parse_dqs2" "\"bar\\nbaz\""
      { statements = [ str "bar\nbaz" ]; prog_loc = Location.dummy_loc };
    tparse "string_parse_sqs1" "'foobar'"
      { statements = [ str "foobar" ]; prog_loc = Location.dummy_loc };
    tparse "string_parse_sqs2" "'bar\\u41'"
      { statements = [ str "barA" ]; prog_loc = Location.dummy_loc };
    tparse "string_parse_sqs3" "'bar\\x41'"
      { statements = [ str "barA" ]; prog_loc = Location.dummy_loc };
    tparse "string_parse_sqs4" "'bar\\101'"
      { statements = [ str "barA" ]; prog_loc = Location.dummy_loc };
    tparse "string_parse_emoji_escape" "\"\xF0\x9F\x98\x82\""
      { statements = [ str "😂" ]; prog_loc = Location.dummy_loc };
    tparse "string_parse_emoji_literal" "\"💯\""
      { statements = [ str "💯" ]; prog_loc = Location.dummy_loc };
    t "string1" "\"foo\"" "\"foo\"";
    t "string2" "\"💯\"" "\"💯\"";
    t "string3" "\"making my way downtown, walking fast\""
...
let tests =
  "End to end"
  >::: basic_functionality_tests @ function_tests @ tuple_tests @ array_tests
       @ record_tests @ stdlib_tests @ box_tests @ loop_tests @ oom @ gc
       @ match_tests @ import_tests @ optimization_tests @ string_tests
       @ data_tests @ export_tests
