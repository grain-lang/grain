...
describe("imports", ({test, testSkip}) => {
  ...
  assertRun(
    "import_all_except",
    {|
      import * except {y} from "exportStar"
      {
        print(x)
        print(z)
      }
  ...
  assertSnapshot(
    ...
    {|
      import * except {x, y} from "exportStar"
      z
    |},
  );
  assertSnapshot(
  ...
  assertCompileError(
    ...
    {|
      import * except {Cons} from "tlists"
      Cons(2, Empty)
    |},
    "Unbound value Cons",
  );
  assertCompileError(
    ...
    {|
      import * except {Cons, append} from "tlists"
      append(Empty, Empty)
    |},
  ...
  assertSnapshot(
    "import_some_mixed",
    {|
      import {Cons, Empty, sum} from "tlists"
      sum(Cons(5, Empty))
    |},
  );
  assertSnapshot(
  ...
  assertSnapshot(
    ...
    {|
      import {x as y, y as x} from "exportStar"
      x(y)
    |},
  );
  assertSnapshot(
    ...
    {|
      import {Empty as None, sum} from "tlists"
      sum(None)
    |},
  );
  assertSnapshot(
    ...
    {|
      import {Cons as Add, Empty as None, sum} from "tlists"
      sum(Add(1, None))
    |},
  );
  /* import {} errors */
  ...
  assertCompileError(
    ...
    {|
      import {Cons as cons, Empty} from "list"
      cons(3, Empty)
    |},
  ...
  assertCompileError(
    ...
    {|
      import {sum as Sum, Empty} from "list"
      sum(Empty)
    |},
  ...
  assertSnapshot(
    ...
    {|
      import TList, { Empty } from "tlists"
      let foo : TList.TList<String> = Empty
      foo
    |},
  );
