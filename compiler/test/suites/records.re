...
describe("records", ({test, testSkip}) => {
  ...
  assertRun(
    "record_2",
    "expose record Rec {foo: Number}; print({foo: 4})",
    "{\n  foo: 4\n}\n",
  );
  assertRun(
    "record_multiple",
    "expose record Rec {foo: Number, bar: String, baz: Bool}; print({foo: 4, bar: \"boo\", baz: true})",
    "{\n  foo: 4,\n  bar: \"boo\",\n  baz: true\n}\n",
  );
  assertSnapshot(
    "record_pun",
    "expose record Rec {foo: Number}; let foo = 4; {foo,}",
  );
  assertSnapshot(
    "record_pun_multiple",
    "expose record Rec {foo: Number, bar: Bool}; let foo = 4; let bar = false; {foo, bar}",
  );
  assertSnapshot(
    "record_pun_mixed",
    "expose record Rec {foo: Number, bar: Bool}; let foo = 4; {foo, bar: false}",
  );
  assertSnapshot(
    "record_pun_mixed_2",
    "expose record Rec {foo: Number, bar: Bool}; let bar = false; {foo: 4, bar}",
  );
  ...
  assertSnapshot(
    ...
    "expose record Rec {foo: Number,}; {foo: 4}",
  );
  assertSnapshot(
  ...
  assertSnapshot(
    ...
    "expose record Rec {foo: Number, bar: String, baz: Bool,}; {foo: 4, bar: \"boo\", baz: true}",
  );
  assertSnapshot(
  ...
  assertRun(
    ...
    {|
      ...
      expose enum Bar { Baz(Foo<Number>) }
      print(Baz({ bar: 1 }))
    |},
    "Baz({\n  bar: 1\n})\n",
  );
