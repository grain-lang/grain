...
let extract_anf { cstate_desc } =
  ...
  | Linearized anf | Optimized anf -> anf
...
let compile_assembly_to_binary asm debug outfile_name =
  let asm_tmp_filename =
    if debug then outfile_name ^ ".wast"
    else temp_file (Filename.basename outfile_name) ".wast"
  in
...
let run_output ?(code = 0) ?heap_size cstate test_ctxt =
  ...
  assert_command
    ...
    ~exit_code:(Unix.WEXITED code) ~use_stderr:true ~ctxt:test_ctxt "grain"
...
let test_run ?cmp ?heap_size program_str outfile expected test_ctxt =
  ...
  in
  assert_equal ~printer:Batteries.identity ~cmp:(Option.default ( = ) cmp)
    (expected ^ "\n") result
