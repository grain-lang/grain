...
let list_printer p l = "[" ^ ExtString.String.join "; " (List.map p l) ^ "]"

let str_printer x = x

let num_printer = string_of_int

let cl_printer elt_printer = sexp_printer (Concatlist.sexp_of_t elt_printer)
...
let test_foldl _ =
  let res = fold_left (fun acc e -> acc ^ string_of_int (e * e)) "B" l1 in
  assert_equal ~printer "B14916" res

let test_foldr _ =
  let res = fold_right (fun e acc -> string_of_int (e * e) ^ acc) l1 "B" in
  assert_equal ~printer "14916B" res

let test_is_empty _ = assert_bool "is_empty" (is_empty (append empty empty))

let test_hd _ =
  let res = hd (append empty (cons 1 empty)) in
  assert_equal ~printer:num_printer 1 res

let test_tl _ =
  let res = tl (cons 1 empty) in
  assert_equal ~printer:cl_num_printer empty res

let test_last _ =
  let res = last @@ append (snoc empty 1) empty in
  assert_equal ~printer:num_printer 1 res

let test_map_to_list _ =
  let aux = ref "" in
  let func e =
    let as_str = string_of_int e in
    aux := !aux ^ as_str;
    as_str
  in
  assert_equal ~printer:(list_printer str_printer) [ "1"; "2"; "3"; "4" ]
