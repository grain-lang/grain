  ...
  Printexc.register_printer (fun exc ->
      ...
      | Some (`Ok err) ->
          ...
          Format.fprintf formatter "@[%a@]@."
            Grain_parsing.Location.report_error err;
...
let output_file_conv =
  let parse s =
    ...
    | true ->
        if Sys.is_directory s_dir then `Ok s
        else `Error (sprintf "`%s' is not a directory" s_dir)
    | false -> `Error (sprintf "no `%s' directory" s_dir)
  in
